  ...
  it("accepts matching topic and lesson parameters and rejects open redirects", () => {
    expect(
      resolveCodeLearningContext(
        "?topicId=topic-b&lessonId=two&returnTo=/learn/topic-b&parameters=%7B%22n%22%3A30%7D",
        lessons,
      ),
    ).toEqual({
      topicId: "topic-b",
      lessonId: "two",
      returnTo: "/learn/topic-b",
      currentParameters: { n: 30 },
      caseId: undefined,
    });
    expect(
...
describe("unified learning progress", () => {
  it("migrates legacy language arrays into the versioned schema", () => {
    ...
    expect(loadLearningProgress(storage)).toMatchObject({
      completedRLessons: ["r-one"],
      completedPythonLessons: ["py-one"],
    });
