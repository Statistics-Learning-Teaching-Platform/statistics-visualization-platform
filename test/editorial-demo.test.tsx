...
describe("warm editorial StatMind demo", () => {
  ...
  it("keeps all four product workspaces in the 2 by 2 editorial index", () => {
    ...
    for (const title of ["统计教学平台", "统计学组卷系统", "R 语言知识库", "Python 语言知识库"]) {
      expect(within(index).getByRole("heading", { name: title })).toBeInTheDocument();
    }
  });
  ...
  it("uses one unambiguous hero action for entering the textbook", () => {
    ...
    expect(screen.getByRole("link", { name: "开始学习" })).toHaveAttribute("href", "/catalog");
  ...
  it("switches the readable textbook chapter without leaving the page", async () => {
    ...
    expect(screen.getByRole("heading", { name: "样本的统计推断" })).toBeInTheDocument();
    await user.click(
      within(screen.getByLabelText("可预览章节")).getByRole("button", {
        name: /总体均值的比较/,
      }),
    );
    expect(screen.getByRole("heading", { name: "总体均值的比较" })).toBeInTheDocument();
  });
  ...
  it("keeps the three-column textbook tools around a rich reading chapter", async () => {
    ...
    expect(
      within(screen.getByRole("navigation", { name: "本章内容导航" })).getAllByRole("link"),
    ).toHaveLength(7);
    expect(screen.queryByText("学习路线")).not.toBeInTheDocument();
    expect(screen.getByLabelText("样本均值标准误公式")).toHaveTextContent("SE( x̄ )");
    ...
    await user.click(within(lab as HTMLElement).getByRole("button", { name: "n = 100" }));
  ...
  it("opens a complete chapter from its query parameter", () => {
    ...
    expect(screen.getByLabelText("独立样本均值差标准误公式")).toBeInTheDocument();
  });
  ...
  it("shows a meaningful empty laboratory state after reset", async () => {
    ...
    expect(screen.getByText(/生成第一个样本后，区间会沿这条总体参照线出现/)).toBeInTheDocument();
  });
  ...
  it("filters and composes the current examination manuscript", async () => {
    ...
    const questionList = screen.getByRole("heading", { name: "已审核题目" }).parentElement
      ?.parentElement;
    expect(questionList).not.toBeNull();
    await user.type(screen.getByRole("textbox", { name: "检索题目" }), "置信区间");
