...
describe("learning progress sync", () => {
  it("merges completed sets and keeps the incoming route", () => {
    ...
    expect([...merged.completedTopics].sort()).toEqual(["descriptive-statistics", "probability"]);
  ...
  it("unions server progress with local progress and pushes the difference", async () => {
    ...
    expect([...stored.completedRLessons].sort()).toEqual(["data-frame-filter", "vectors-and-mean"]);
  ...
  it("shows account-synced evidence on the profile page", async () => {
    ...
    await waitFor(() => expect(screen.getByText("1 / 39 已完成")).toBeInTheDocument());
