...
describe("textbook resource catalog", () => {
	...
	it("keeps all twelve chapters on one page and aggregates every coding lesson once", () => {
		...
		expect(
			textbookResourceCatalog.flatMap(({ rLessons: lessons }) => lessons),
		).toHaveLength(rLessons.length);
		expect(
			textbookResourceCatalog.flatMap(({ pythonLessons: lessons }) => lessons),
		).toHaveLength(pythonLessons.length);
	});
	...
	it("opens with a full-page chapter index, then enters the chapter learning hub", async () => {
		...
		const chapterIndex = screen.getByLabelText("完整教材章节目录");
		...
		await user.click(
			within(chapterIndex).getByRole("button", { name: /06 样本的统计推断/ }),
		);
