...
import {
  inferTopicIds,
  topicLabels,
} from "../integrations/st-qselector/Program/src/lib/topic-mapping";
import {
...
function readServerQuestionIndex(): {
  ...
  const file = path.resolve(
    "integrations/st-qselector/Program/src/generated/reviewed-questions.ts",
  );
...
describe("question bank topic mapping", () => {
  ...
  it("only exposes topic IDs registered by the course", () => {
    ...
    expect(inferTopicIds("Ch06", ["central limit theorem", "sampling distribution"])).toEqual(
      expect.arrayContaining(["central-limit-theorem", "sampling-distributions"]),
    );
  });
