...
import type { ControlConfig, ControlValue, ExampleConfig, ModuleConfig, State } from "./types";
...
export function normalizeControls(
  ...
  for (const control of example.controls) {
    ...
    if (effective.type !== "number" || effective.min === undefined || effective.max === undefined)
      continue;
...
function renderControl(
  ...
  const dependentValue = control.labelByValue
    ? String(controls[control.labelByValue.controlId] ?? "")
    : "";
  const visibleLabel = control.labelByValue?.labels[dependentValue] ?? control.label;
  const description =
    control.description ??
    (effectiveControl.type === "number" &&
    effectiveControl.min !== undefined &&
    effectiveControl.max !== undefined
      ? language === "zh"
      ...
      : effectiveControl.type === "select"
        ? language === "zh"
          ? "选择当前实验的模式或数据设置。"
          : "Choose the current experiment mode or data setting."
        : language === "zh"
          ? "输入当前实验使用的数据。"
          : "Enter the data used by this experiment.");
    ...
    <label className="control-field" key={control.id}>
      ...
      <input
        ...
        onChange={(e) =>
          onChange(
            control.id,
            effectiveControl.type === "number" ? Number(e.target.value) : e.target.value,
          )
        }
      />
      {!compact && <span className="control-description">{description}</span>}
...
const formulaRenderers: Record<string, (state: State) => ReactNode> = {
  ...
  buffon: () => (
    <div className="math-expression">
      <span>π ≈</span>
      <span className="math-frac">
        <span className="math-num">2 L N</span>
        <span className="math-den">T · C</span>
      </span>
    </div>
  ),
  "random-normal": () => (
  ...
  "gamma-rejection": (state) => (
    <div className="math-expression">
      <span>
        {state.language === "zh"
          ? "接受 x ∼ g(x)，概率为 f(x) / M g(x)"
          : "accept x ∼ g(x) with probability f(x) / M g(x)"}
      </span>
    </div>
  ),
  "bootstrap-max": (state) => (
    <div className="math-expression">
      <span>θ̂* = T(X₁*, …, Xₙ*)</span>
      <span className="math-symbol">,</span>
      <span>{state.language === "zh" ? "Xᵢ* ∼ 经验数据" : "Xᵢ* ∼ empirical data"}</span>
    </div>
  ),
  "mean-bootstrap": () => (
    <div className="math-expression">
      <span>SE_boot = sd(θ̂*)</span>
    </div>
  ),
  "mc-integral-exp": () => (
    <div className="math-expression">
      <span>∫ f(x)dx ≈ (1 / N)</span>
      <span className="math-symbol">Σ</span>
      <span>f(Xᵢ)</span>
    </div>
  ),
  ...
  "control-exp": () => (
    <div className="math-expression">
      <span>Y_cv = Y + c(C − E[C])</span>
    </div>
  ),
  "control-ratio": () => (
    <div className="math-expression">
      <span>c* = −Cov(Y, C) / Var(C)</span>
    </div>
  ),
  "importance-power": () => (
    <div className="math-expression">
      <span>E_f[h(X)] = E_g[h(X) f(X) / g(X)]</span>
    </div>
  ),
  "conditional-circle": () => (
    <div className="math-expression">
      <span>E[Y] = E[E(Y | X)]</span>
    </div>
  ),
  politician: (state) => (
    <div className="math-expression">
      <span>
        {state.language === "zh"
          ? "P(接受) = min(1, N提议 / N当前)"
          : "P(accept) = min(1, Nproposal / Ncurrent)"}
      </span>
    </div>
  ),
};
...
function renderFormula(state: State): ReactNode {
  ...
  return (
    <span>
      {state.language === "zh"
        ? "请结合图表中的当前统计量阅读本实验。"
        : "Read the current statistic alongside the chart."}
    </span>
  );
}
...
export function WalsApp({ moduleConfig }: WalsAppProps) {
  ...
  const state = useMemo(
    () =>
      createState(
        moduleConfig,
        exampleId,
        stochastic ? resultControls : deferredControls,
        deferredSeed,
        language,
        sampleMeans,
      ),
  ...
  const handleSelectExample = useCallback(
    (id: string) => {
      ...
      setAppliedControls(
        metadata?.interaction === "stochastic" && nextExample
          ? createDefaultControls(nextExample)
          : undefined,
      );
      setChangeSummary(null);
      setSeed((s) => s + 1);
      // Reset to the lazy "not yet drawn" state; the CLT engine regenerates a
      // deterministic default sample from the new seed on demand.
  ...
  const handleUpdateControl = useCallback(
    (id: string, value: ControlValue) => {
      ...
      if (!stochastic && (!isProbability || probabilitySummaryControl)) {
        ...
        if (previous !== value) {
          const label =
            state.activeExample.controls.find((control) => control.id === id)?.label ?? id;
          setChangeSummary({
            parameterId: id,
            parameter: label,
            previousValue: previous,
            currentValue: value,
            previousMetric: state.result.metrics[0]?.value,
          });
        }
      }
      setControls((prev) => {
  ...
  const handleBumpControl = useCallback(
  ...
  const handleDrawSamples = useCallback(
    (count: number) => {
      ...
      setSampleMeans(
        combined.length > MAX_SAMPLE_MEANS
          ? combined.slice(combined.length - MAX_SAMPLE_MEANS)
          : combined,
      );
      if (stochastic) setAppliedControls(current);
      setSeed(nextSeed);
    },
    [controls, state.activeExample, sampleMeans, seed, stochastic],
  );
  ...
  const groupedControls = useMemo(() => {
    ...
    const visible = state.activeExample.controls.filter(
      (control) =>
        !control.hideWhen?.values.includes(
    ...
    if (isProbability) {
      return {
        ...
        display: visible.filter(
          (control) => control.id === "mode" || control.id === "showReference",
        ),
      };
    }
    return {
      mode: visible.filter(
        (control) =>
          control.group === "mode" ||
          control.id === "mode" ||
          (control.type === "select" && visible.indexOf(control) === 0),
      ),
      core: visible.filter(
        (control) =>
          control.group === "core" ||
  ...
  const changeSummaryNode =
    changeSummary && !stochastic ? (
      <ExperimentChangeSummary
        eyebrow={isProbability ? (language === "zh" ? "参数变化" : "Parameter change") : undefined}
        parameter={changeSummary.parameter}
        previousValue={
          isProbability
            ? formatProbabilityChangeValue(changeSummary.parameterId, changeSummary.previousValue)
            : String(changeSummary.previousValue)
        }
        currentValue={
          isProbability
            ? formatProbabilityChangeValue(changeSummary.parameterId, changeSummary.currentValue)
            : String(changeSummary.currentValue)
        }
        ...
        interpretation={
          isProbability
            ? probabilityChangeInterpretation(
                changeSummary,
                language,
                String(state.controls.dist ?? "norm"),
              )
            : language === "zh"
              ? "参数变化已即时反馈到当前指标与图形。"
  ...
  const parameterPanelNode = (
    <ParameterPanel
      eyebrow={isAnova ? (language === "zh" ? "数据集" : "Dataset") : state.copy.parameters}
      className={
        isAnova ? "anova-parameter-rail" : isProbability ? "probability-parameter-panel" : ""
      }
    >
      ...
      {groupedControls.mode.length > 0 && (
        <ControlGroup
          className={
            isProbability ? "probability-mode-group" : isAnova ? "anova-dataset-group" : ""
          }
          title={
            isProbability
              ? ""
        ...
        >
          <div className="control-grid">
            {groupedControls.mode.map((control) =>
              renderControl(
                control,
                controls ?? createDefaultControls(state.activeExample),
                handleUpdateControl,
                language,
                isProbability,
              ),
      ...
      {isProbability && groupedControls.display.length > 0 && (
        <ControlGroup
          className="probability-display-group"
          title={language === "zh" ? "显示" : "Display"}
        >
          <div className="control-grid">
            {groupedControls.display.map((control) =>
              renderControl(
                control,
                controls ?? createDefaultControls(state.activeExample),
                handleUpdateControl,
      ...
      {groupedControls.core.length > 0 && (
        <ControlGroup
          className={isProbability ? "probability-core-group" : isAnova ? "anova-core-group" : ""}
          title={
            isProbability
              ? ""
              : isAnova
        ...
        >
          <div className="control-grid">
            {groupedControls.core.map((control) =>
              renderControl(
                control,
                controls ?? createDefaultControls(state.activeExample),
                handleUpdateControl,
      ...
      {!isProbability && groupedControls.display.length > 0 && (
        <ControlGroup
          className={isProbability ? "probability-display-group" : ""}
          title={
            isProbability
              ? language === "zh"
                ? "显示"
      ...
      {stochastic && (
        <RunControls>
          ...
          <span className="run-controls__hint">
            {language === "zh"
              ? "参数改变后，点击运行才会生成新的随机结果。"
              : "Change parameters, then run to generate new random results."}
          </span>
        </RunControls>
      )}
      {isProbability && (
        ...
        <div className="sample-quick-actions">
          {quickActions
            .filter(
              (action) =>
                !action.showWhen ||
                action.showWhen.values.includes(
                  String(state.controls[action.showWhen.controlId] ?? ""),
                ),
            )
            .map((action) => (
              <button
                ...
                onClick={() => {
                  if (action.type === "drawSampleMeans") {
                    ...
                    const next = normalizeControls(
                      state.activeExample,
                      { ...current, [controlId]: action.amount },
                      controlId,
                    );
                    setControls(next);
  ...
  const anovaStatisticsNode = (
    <div className="anova-statistics-stack">
      {(state.result.tables ?? (state.result.table ? [state.result.table] : [])).map(
        (table, index) => (
          <section
            className="teaching-panel table-panel anova-table-panel"
          ...
          >
            ...
            <StatisticsTable
              ...
              rows={table.rows.map((row, rowIndex) => [
                ...
                ...(index === 1
                  ? [rowIndex === 0 ? (state.result.metrics[1]?.value ?? "") : ""]
                  : []),
              ])}
            />
          </section>
      ...
      content={
        <>
          ...
          <section className={`output-dock${isProbability ? " probability-output" : ""}`}>
            ...
            {isAnova && (
              <section
                className="anova-inline-results"
                aria-label={language === "zh" ? "方差分析结果" : "ANOVA results"}
              >
                {anovaStatisticsNode}
              </section>
            )}
...
function probabilityChangeInterpretation(
  ...
  if (language === "en") {
    ...
    if (change.parameterId === "b" && distribution === "norm")
      return "The variance changes with σ², widening or narrowing the curve.";
    if (change.parameterId === "lower" || change.parameterId === "upper")
      return "The interval boundary and highlighted probability area update together.";
    if (change.parameterId === "dist")
      return "The distribution family changes, updating the curve shape and moments.";
  ...
  if (change.parameterId === "b" && distribution === "norm")
    return "方差随 σ² 变化，曲线宽度与峰高同步改变。";
  if (change.parameterId === "lower" || change.parameterId === "upper")
    return "区间端点与橙色高亮区域、区间概率同步更新。";
...
function formatProbabilityDistribution(controls: Record<string, ControlValue>): string {
  ...
  switch (dist) {
    case "norm":
      return `N(${fixed(a)}, ${fixed(b)}²)`;
    case "t":
      return `t(df=${Math.round(a)})`;
    case "beta":
      return `Beta(${fixed(a)}, ${fixed(b)})`;
    case "gamma":
      return `Gamma(${fixed(a)}, rate=${fixed(b)})`;
    case "chisq":
      return `χ²(df=${Math.round(b)})`;
    case "exp":
      return `Exp(λ=${fixed(b)})`;
    case "unif":
      return `U(${fixed(Math.min(a, b))}, ${fixed(Math.max(a, b))})`;
    case "binom":
      return `Bin(n=${Math.round(b)}, p=${fixed(a)})`;
    case "geom":
      return `Geom(p=${fixed(a)})`;
    case "pois":
      return `Pois(λ=${fixed(b)})`;
    default:
      return dist;
  }
}
...
function formatProbabilityChangeValue(parameterId: string, value: ControlValue): string {
  if (
    ["a", "b", "lower", "upper"].includes(parameterId) &&
    typeof value === "number" &&
