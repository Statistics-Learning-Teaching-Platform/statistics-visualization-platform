...
export function antitheticExp(controls: ControlMap, seed: number): SimulationResult {
  ...
  return result(
    ...
    { type: "scatter", title: "Antithetic paired values", xLabel: "f(U)", yLabel: "f(1 − U)", points: pairedPoints.map(({ x, y }) => ({ x, y, color: "var(--lab-teal)" })), references: [{ axis: "y", value: exact, label: "E[f(U)]", color: "var(--lab-purple)" }], legend: [{ label: "paired draws", color: "var(--lab-teal)", shape: "dot" }] }
  );
}
