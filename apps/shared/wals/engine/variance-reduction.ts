...
export function antitheticExp(controls: ControlMap, seed: number): SimulationResult {
  ...
  const pairCorrelation =
    pairedPoints.length > 1
      ? pairedPoints.reduce(
          (sum, point) => sum + (point.x - pairedMeanX) * (point.y - pairedMeanY),
          0,
        ) /
        (pairedPoints.length - 1) /
        Math.sqrt(
          Math.max(
            Number.EPSILON,
            variance(pairedPoints.map((point) => point.x)) *
              variance(pairedPoints.map((point) => point.y)),
          ),
        )
      : 0;
  return result(
    ...
    [
      {
        ...
        value: formatNumber(mean(paired), 5),
        detail: `exact ${formatNumber(exact, 5)}`,
      },
      {
        label: "independent estimate",
        value: formatNumber(mean(independent), 5),
        detail: "simple MC",
      },
      {
        label: "variance reduction",
        value: reductionPct(pairedEstimatorVariance, independentEstimatorVariance),
        detail: `${n} function evaluations each`,
      },
      {
        label: "pair correlation",
        value: formatNumber(pairCorrelation, 3),
        detail: "correlation of f(U) and f(1 − U)",
      },
    ],
    {
      type: "bars",
      title: "Variance under the same simulation budget",
      xLabel: "estimator",
      yLabel: "estimator variance",
      bars: [
        {
          label: "antithetic",
          value: pairedEstimatorVariance,
          color: "#2f6f64",
          semanticColor: "var(--lab-teal)",
        },
        {
          label: "independent",
          value: independentEstimatorVariance,
          color: "#c8665a",
          semanticColor: "var(--lab-coral)",
        },
      ],
    },
  );
  ...
  return result(
    ...
    [
      { label: "estimate", value: formatNumber(mean(paired), 5), detail: "antithetic method" },
      {
        ...
        value: formatNumber(standardDeviation(paired) / Math.sqrt(half), 5),
        detail: `${half} pairs`,
      },
      {
        label: "variance reduction",
    ...
    {
      type: "bars",
      title: "Pair-mean histogram",
      xLabel: "pair mean",
      yLabel: "count",
      bars: histogram(paired),
    },
  );
}
    ...
    [
      {
        ...
        value: formatNumber(mean(simpleValues), 5),
        detail: `variance ${formatNumber(variance(simpleValues), 5)}`,
      },
      {
        label: "control estimate",
        value: formatNumber(mean(controlValues), 5),
        detail: `variance ${formatNumber(variance(controlValues), 5)}`,
      },
      {
        label: "variance reduction",
        value: reductionPct(variance(controlValues), variance(simpleValues)),
        detail: `c = ${formatNumber(coefficient, 3)}`,
      },
      {
        label: "optimal coefficient",
        value: formatNumber(optimalCoefficient, 4),
        detail: "theoretical minimum-variance c",
      },
    ],
    {
      type: "bars",
      title: "Simple versus control-variate variance",
      xLabel: "estimator",
      yLabel: "sample variance",
      bars: [
        { label: "simple", value: variance(simpleValues), color: "var(--lab-coral)" },
        { label: "control variate", value: variance(controlValues), color: "var(--lab-teal)" },
      ],
    },
  );
}
export function controlRatio(controls: ControlMap, seed: number): SimulationResult {
  ...
  const covariance = mean(
    pilot.map((_, index) => (pilotF[index] - fMean) * (pilotG[index] - gMean)),
  );
  ...
  return result(
    ...
    [
      {
        label: "simple estimate",
        value: formatNumber(mean(simple), 5),
        detail: `se ${formatNumber(standardDeviation(simple) / Math.sqrt(n), 5)}`,
      },
      {
        label: "controlled estimate",
        value: formatNumber(mean(controlled), 5),
        detail: `se ${formatNumber(standardDeviation(controlled) / Math.sqrt(n), 5)}`,
      },
      {
        label: "variance reduction",
        value: reductionPct(variance(controlled), variance(simple)),
        detail: `c = ${formatNumber(c, 3)}`,
      },
    ],
    {
      type: "bars",
      title: "Variance after estimating a control coefficient",
      xLabel: "estimator",
      yLabel: "sample variance",
      bars: [
        { label: "simple", value: variance(simple), color: "var(--lab-coral)" },
        { label: "controlled", value: variance(controlled), color: "var(--lab-teal)" },
      ],
    },
  );
}
export function importancePower(controls: ControlMap, seed: number): SimulationResult {
  ...
  const ess =
    ...
    Math.max(
      Number.EPSILON,
      importanceWeights.reduce((sum, weight) => sum + weight * weight, 0),
    );
  return result(
    ...
    [
      {
        ...
        value: formatNumber(mean(importance), 6),
        detail: `variance ${formatNumber(variance(importance), 6)}`,
      },
      {
        label: "simple estimate",
        value: formatNumber(mean(simple), 6),
        detail: `variance ${formatNumber(variance(simple), 6)}`,
      },
      { label: "exact target", value: formatNumber(exact, 6), detail: "1 / 6.1" },
      {
        label: "variance reduction",
        value: reductionPct(variance(importance), variance(simple)),
        detail: "importance vs simple",
      },
      {
        label: "ESS",
        value: formatNumber(ess, 0),
        detail: "effective sample size from proposal weights",
      },
    ],
    {
      type: "scatter",
      title: "Target versus proposal contributions",
      xLabel: "uniform contribution",
      yLabel: "importance contribution",
      points: importance
        .slice(0, 250)
        .map((value, index) => ({ x: simple[index] ?? 0, y: value, color: "var(--lab-teal)" })),
      legend: [{ label: "proposal-weighted draw", color: "var(--lab-teal)", shape: "dot" }],
    },
  );
}
    ...
    [
      {
        ...
        value: formatNumber(mean(conditional), 5),
        detail: `variance ${formatNumber(variance(conditional), 5)}`,
      },
      {
        label: "simple estimate",
        value: formatNumber(mean(simple), 5),
        detail: `variance ${formatNumber(variance(simple), 5)}`,
      },
      { label: "exact target", value: formatNumber(exact, 5), detail: "π / 4" },
      {
        label: "variance reduction",
        value: reductionPct(variance(conditional), variance(simple)),
        detail: "conditional vs simple",
      },
    ],
    {
      type: "bars",
      title: "Conditioning smooths a hit-or-miss estimator",
      xLabel: "estimator",
      yLabel: "sample variance",
      bars: [
        { label: "conditional", value: variance(conditional), color: "var(--lab-teal)" },
        { label: "hit or miss", value: variance(simple), color: "var(--lab-coral)" },
      ],
    },
  );
}
