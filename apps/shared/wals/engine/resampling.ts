...
export function permutationHistogram(
  ...
  return bins.flatMap((bin) => {
    const segments = [
      bin.centralCount > 0
        ? {
            x: bin.centralSum / bin.centralCount,
            value: bin.centralCount,
            color: "#2f6f64" as const,
          }
        : undefined,
      bin.tailCount > 0
...
export function bootstrapMax(controls: ControlMap, seed: number): SimulationResult {
  ...
  const inputWasLimited =
    rawData.length > boundedData.length || parsedValues.length > values.length;
  ...
  if (values.length === 0) {
    return result(
      ...
      ],
      {
        type: "bars",
        title: "No data",
        xLabel: "max value bin",
        yLabel: "count",
        bars: [{ label: "n/a", value: 0 }],
      },
    );
  }
  ...
  return result(
    ...
    [
      {
        ...
        value: formatNumber(observed, 4),
        detail: "maximum of observed data",
      },
      {
        label: "bootstrap mean",
        value: formatNumber(bootstrapMean, 4),
        detail: "mean of bootstrap maxima",
      },
      {
        label: "bias",
        value: formatNumber(bootstrapMean - observed, 4),
        detail: "bootstrap mean − observed",
      },
      {
        label: "bootstrap SE",
        value: formatNumber(standardDeviation(maxima), 4),
        detail: "SD of bootstrap statistics",
      },
      {
        label: "95% CI",
        value: `${formatNumber(ciLower, 3)} - ${formatNumber(ciUpper, 3)}`,
        detail: "percentile interval",
      },
      {
        label: "replicates",
    ...
    {
      type: "bars",
      title: "Bootstrap maxima",
      xLabel: "max value bin",
      yLabel: "count",
      bars: histogram(maxima),
      references: [
    ...
    [
      {
        label: "observed statistic",
        value: formatNumber(observed, 4),
        detail: "mean of observed sample",
      },
      {
        label: "bootstrap mean",
        value: formatNumber(bootstrapMean, 4),
        detail: "mean of bootstrap means",
      },
      {
        label: "bias",
        value: formatNumber(bootstrapMean - observed, 4),
      ...
      {
        label: "bootstrap SE",
        value: formatNumber(standardDeviation(bootMeans), 4),
        detail: "SD of bootstrap statistics",
      },
      {
        label: "95% CI",
    ...
    [
      {
        ...
        value: formatNumber(observed, 4),
        detail: "mean(B) - mean(A)",
      },
      {
        label: "two-sided p value",
        value: formatNumber(pValue, 4),
        detail: `${extreme} extreme permutations`,
      },
      {
        label: "permutations",
        value: String(replicates),
        detail: `${groupSize} observations per group`,
      },
      {
        label: "decision at 5%",
        value: pValue < 0.05 ? "reject H0" : "do not reject H0",
        detail: "based on permutation tail area",
      },
    ],
    {
      type: "bars",
