  ...
  return {
    ...result(
      ...
      [
        {
          label: "sample mean",
          value: formatNumber(mean(sample), 4),
          detail: `target ${formatNumber(targetMean, 2)}`,
        },
        {
          label: "sample SD",
          value: formatNumber(standardDeviation(sample), 4),
          detail: `target ${formatNumber(sd, 2)}`,
        },
        {
          label: "theoretical mean",
          value: formatNumber(targetMean, 4),
          detail: "population parameter",
        },
        { label: "theoretical SD", value: formatNumber(sd, 4), detail: "population parameter" },
        {
          label: "mean error",
          value: formatNumber(mean(sample) - targetMean, 4),
          detail: "sample mean − theoretical mean",
        },
      ],
      {
        type: "bars",
        title: "Normal sample histogram",
        xLabel: "bin center",
        yLabel: "count",
        bars: histogram(sample),
      },
    ),
    rawSample: sample,
  };
      ...
      [
        {
          label: "sample mean",
          value: formatNumber(mean(sample), 4),
          detail: `theory ${formatNumber(1 / lambda, 4)}`,
        },
        {
          label: "sample SD",
          value: formatNumber(standardDeviation(sample), 4),
        ...
        {
          label: "mean error",
          value: formatNumber(mean(sample) - 1 / lambda, 4),
          detail: "sample mean − theoretical mean",
        },
      ],
      {
        type: "bars",
        title: "Exponential sample histogram",
        xLabel: "bin center",
        yLabel: "count",
        bars: histogram(sample),
      },
...
export function gammaRejection(controls: ControlMap, seed: number): SimulationResult {
  ...
  const drawShapeUnitRate = (): { value: number; proposals: number } => {
    ...
    while (true) {
      ...
      if (u < 1 - 0.0331 * z ** 4 || Math.log(u) < 0.5 * z * z + d * (1 - v + Math.log(v))) {
    ...
    ...result(
      ...
      [
        { label: "accepted", value: String(accepted.length), detail: `${proposals} candidates` },
        {
          ...
          value: formatNumber(accepted.length / proposals, 4),
          detail: "accepted / candidates",
        },
        {
          label: "accepted mean",
          value: formatNumber(acceptedMean, 4),
          detail: `gamma mean ${formatNumber(alpha / beta, 4)}`,
        },
        {
          label: "proposal support",
          value: "(0, ∞)",
          detail: alpha < 1 ? "GS envelope" : "Marsaglia-Tsang envelope",
        },
      ],
      {
        type: "bars",
        title: "Accepted sample histogram",
        xLabel: "bin center",
        yLabel: "count",
        bars: histogram(accepted),
      },
    ),
    rawSample: accepted,
  };
