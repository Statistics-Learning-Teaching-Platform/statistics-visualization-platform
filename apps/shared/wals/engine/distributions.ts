...
export function distribution(controls: ControlMap): SimulationResult {
  ...
  const moments: { mean: number; variance: number } = (() => {
    switch (dist) {
      case "norm":
        return { mean: a, variance: b ** 2 };
      case "t":
        return {
          mean: params.tDf > 1 ? 0 : Number.NaN,
          variance: params.tDf > 2 ? params.tDf / (params.tDf - 2) : Number.POSITIVE_INFINITY,
        };
      case "beta":
        return {
          mean: params.shape / (params.shape + params.betaParam),
          variance:
            (params.shape * params.betaParam) /
            ((params.shape + params.betaParam) ** 2 * (params.shape + params.betaParam + 1)),
        };
      case "gamma":
        return { mean: params.shape / b, variance: params.shape / b ** 2 };
      case "chisq":
        return { mean: params.chisqDf, variance: 2 * params.chisqDf };
      case "exp":
        return { mean: 1 / b, variance: 1 / b ** 2 };
      case "unif":
        return {
          mean: (uniformLower + uniformUpper) / 2,
          variance: (uniformUpper - uniformLower) ** 2 / 12,
        };
      case "pois":
        return { mean: b, variance: b };
      case "binom":
        return {
          mean: params.binomN * params.prob,
          variance: params.binomN * params.prob * (1 - params.prob),
        };
      case "geom":
        return {
          mean: (1 - params.prob) / params.prob,
          variance: (1 - params.prob) / params.prob ** 2,
        };
      default:
        return { mean: a, variance: b ** 2 };
    }
  ...
  const pdfAt = (x: number): number => {
    switch (dist) {
      ...
      case "t":
        return jStat.studentt.pdf(x, params.tDf);
      case "beta":
        return x > 0 && x < 1 ? jStat.beta.pdf(x, params.shape, params.betaParam) : 0;
      case "gamma":
        return x > 0 ? jStat.gamma.pdf(x, params.shape, 1 / b) : 0; // jstat takes (shape, scale = 1/rate)
      case "chisq":
        return x > 0 ? jStat.chisquare.pdf(x, params.chisqDf) : 0;
      case "exp":
        return x < 0 ? 0 : b * Math.exp(-b * x);
      case "unif":
        return x >= uniformLower && x <= uniformUpper
          ? 1 / Math.max(uniformUpper - uniformLower, Number.EPSILON)
          : 0;
      case "pois":
        return x >= 0 && Number.isInteger(x) ? jStat.poisson.pdf(x, b) : 0;
      case "binom":
        return x >= 0 && x <= params.binomN && Number.isInteger(x)
          ? jStat.binomial.pdf(x, params.binomN, params.prob)
          : 0;
      case "geom":
        return x >= 0 && Number.isInteger(x) ? Math.pow(1 - params.prob, x) * params.prob : 0;
      default:
        return normalPdf(x, a, b);
    }
  };
  ...
  const cdfAt = (x: number): number => {
    switch (dist) {
      case "norm":
        return normalCdf(x, a, b);
      case "t":
        return jStat.studentt.cdf(x, params.tDf);
      case "beta":
        return x <= 0 ? 0 : x >= 1 ? 1 : jStat.beta.cdf(x, params.shape, params.betaParam);
      case "gamma":
        return x <= 0 ? 0 : jStat.gamma.cdf(x, params.shape, 1 / b);
      case "chisq":
        return x <= 0 ? 0 : jStat.chisquare.cdf(x, params.chisqDf);
      case "exp":
        return x < 0 ? 0 : 1 - Math.exp(-b * x);
      case "unif":
        return x < uniformLower
          ? 0
          : x > uniformUpper
            ? 1
            : (x - uniformLower) / Math.max(uniformUpper - uniformLower, Number.EPSILON);
      case "pois":
        return x < 0 ? 0 : jStat.poisson.cdf(Math.floor(x), b);
      case "binom":
        return x < 0
          ? 0
          : x >= params.binomN
            ? 1
            : jStat.binomial.cdf(Math.floor(x), params.binomN, params.prob);
      case "geom":
        return x < 0 ? 0 : 1 - Math.pow(1 - params.prob, Math.floor(x) + 1);
      default:
        return normalCdf(x, a, b);
    }
  };
  ...
  const discreteMax =
    dist === "binom"
      ...
      : dist === "pois"
        ...
        : Math.min(
            100,
            Math.max(12, Math.ceil(Math.log(0.001) / Math.log(Math.max(1 - params.prob, 0.001)))),
          );
  ...
  const continuousMin =
    dist === "norm" ? -12 : Math.min(intervalLower, support[0] - supportWidth * 0.04);
  const continuousMax =
    dist === "norm" ? 12 : Math.max(intervalUpper, support[1] + supportWidth * 0.04);
  const xs = isDiscrete
    ? Array.from({ length: discreteMax + 1 }, (_, index) => index)
    : Array.from(
        { length: 180 },
        (_, index) => continuousMin + (index * (continuousMax - continuousMin)) / 179,
      );

  const points = xs.map((x) => ({ x, y: mode === "CDF" ? cdfAt(x) : pdfAt(x) }));
  const normalReferencePoints =
    dist === "norm" && showReference
  ...
  return result(
    ...
    [
      ...
      { label: "Var(X)", value: formatNumber(moments.variance, 4), detail: "theoretical variance" },
      {
        ...
        value: formatNumber(probability, 4),
        detail: `P(${intervalLower} <= X <= ${intervalUpper})`,
      },
    ...
    isDiscrete
      ? {
          ...
          bars: points.map((point) => ({
            ...
            value: point.y,
            color:
              point.x >= intervalLower && point.x <= intervalUpper
                ? "var(--lab-orange)"
                : "var(--lab-teal)",
          })),
          legend: [
            ...
            dist === "norm"
              ? [
                  ...
                  ...(showReference
                    ? [
                        {
                          label: "reference N(0, 1)",
                          points: normalReferencePoints,
              ...
              ? [
                  {
                    ...
                    points: points.filter(
                      (point) => point.x >= intervalLower && point.x <= intervalUpper,
                    ),
