...
import { area, line, type ScaleLinear, scaleBand, scaleLinear } from "d3";
import type { ChartLegendItem, ChartPoint, ChartReference, ChartSeries, ChartSpec } from "./types";
          ...
          <g key={`${item.label}-${index}`} transform={`translate(${x}, 16)`}>
            ...
            {item.shape === "dot" ? (
              ...
              <line
                ...
                stroke={item.color}
                strokeWidth={3}
                strokeDasharray={item.shape === "dashed" ? "4 3" : undefined}
              />
            )}
  ...
  return (
    <Frame title={spec.title} xLabel={spec.xLabel} yLabel={spec.yLabel}>
      ...
      {spec.circles?.map((circle, index) => (
        <g key={`circle-${index}`}>
          ...
          {circle.label && (
            <text
              x={x(circle.cx)}
              y={y(circle.cy - circle.radius) - 7}
              textAnchor="middle"
              fill={circle.color ?? chartTheme.lavender}
              fontSize={11}
              fontWeight={750}
            >
              {circle.label}
      ...
      {contours.map((series, i) => (
        <path
          key={`contour-${i}`}
          d={lineGen(series.points) ?? ""}
          fill="none"
          stroke={series.color ?? chartTheme.lavender}
          strokeWidth={1.4}
          strokeOpacity={series.opacity ?? 0.62}
      ...
      {lines.map((series, i) => (
        <path
          ...
          d={lineGen(series.points) ?? ""}
          fill="none"
          stroke={series.color ?? chartTheme.danger}
          strokeWidth={3}
      ...
      {spec.points.map((point, i) => (
        <g key={i}>
          <circle
            ...
            cy={y(point.y)}
            r={point.label ? 4.5 : 3}
            fill={point.color ?? chartTheme.teal}
            opacity={0.78}
          >
            <title>
              {point.label ? `${point.label}: ` : ""}(${Number(point.x.toFixed(3))}, $
              {Number(point.y.toFixed(3))})
            </title>
          </circle>
          {spec.showPointLabels && point.label && (
            <text x={x(point.x) + 7} y={y(point.y) - 7} fill={chartTheme.text} fontSize={10}>
              {point.label}
            </text>
          )}
        </g>
      ...
      <Legend
        items={
          ...
          lines.map((series) => ({
            label: series.label,
            color: series.color ?? chartTheme.danger,
            shape: series.dashed ? "dashed" : "line",
          }))
        }
      />
    </Frame>
    ...
    <Frame title={spec.title} xLabel={spec.xLabel} yLabel={spec.yLabel}>
      ...
      {spec.series.map((series, i) => (
        <path
          ...
          d={lineGen(series.points) ?? ""}
          fill="none"
          stroke={series.color ?? chartTheme.teal}
          strokeWidth={3}
          strokeOpacity={series.opacity ?? 1}
          strokeDasharray={series.dashed ? "7 5" : undefined}
        />
      ...
      {(spec.areas ?? []).map((shape, i) => {
        ...
        return (
          <path
            key={`area-${i}`}
            d={areaGen(shape.points) ?? ""}
            fill={shape.color ?? chartTheme.orange}
      ...
      <Legend
        items={
          ...
          spec.series.map((series) => ({
            label: series.label,
            color: series.color ?? chartTheme.teal,
            shape: series.dashed ? "dashed" : "line",
          }))
        }
      />
...
function BarsChart({ spec }: { spec: Extract<ChartSpec, { type: "bars" }> }) {
  ...
  const x = scaleBand<number>()
    .domain(spec.bars.map((_, index) => index))
    .range([margin.left, width - margin.right])
    .padding(0.18);
  const y = scaleLinear()
    .domain(yDomain)
  ...
  const numericX = numericLabels.every(Number.isFinite)
    ? scaleLinear()
        .domain(extent(numericLabels, [0, 1]))
        .nice()
        .range([margin.left, width - margin.right])
    : undefined;
  return (
    <Frame title={spec.title} xLabel={spec.xLabel} yLabel={spec.yLabel}>
      <GridLines y={y} />
      <AxisTicks scale={y} orientation="y" />
      {numericX && spec.references && (
        <References references={spec.references} x={numericX} y={y} />
      )}
      {spec.bars.map((bar, i) => (
        <g key={i}>
          <rect
            ...
            fill={bar.semanticColor ?? bar.color ?? chartTheme.teal}
            opacity={0.86}
          >
          ...
          <text
            ...
            transform={
              spec.bars.length > 8
                ? `rotate(-35 ${(x(i) ?? margin.left) + x.bandwidth() / 2} ${height - margin.bottom + 15})`
                : undefined
            }
            fill={chartTheme.muted}
...
function IntervalsChart({ spec }: { spec: Extract<ChartSpec, { type: "intervals" }> }) {
  ...
  const intervalStrokeWidth =
    rowSpacing < 4 ? 1.6 : rowSpacing < 8 ? 2.7 : rowSpacing < 12 ? 3.8 : 5;
      ...
      {spec.intervals.map((interval, i) => {
        ...
        return (
          <g
            ...
            key={i}
            aria-label={`${interval.label}: ${interval.lower} to ${interval.upper}`}
          >
            ...
            <line
              ...
              y1={yCenter}
              y2={yCenter}
              stroke={interval.color ?? chartTheme.teal}
            ...
            <circle
              ...
              cy={yCenter}
              r={centerRadius}
              fill={interval.color ?? chartTheme.teal}
            />
            {showLabel && (
              <text
                className="interval-row__label"
                x={margin.left - 10}
                y={yCenter}
...
function CltChart({ spec }: { spec: Extract<ChartSpec, { type: "clt" }> }) {
  ...
  const popY = scaleLinear()
    .domain([0, populationMax * 1.15])
    .range([top.y + top.height, top.y]);
  const meanY = scaleLinear()
    .domain([0, samplingMax * 1.18])
    .range([bottom.y + bottom.height, bottom.y]);
  ...
  const normalLine = line<ChartPoint>()
    .x((p) => x(p.x))
    .y((p) => meanY(p.y));
  ...
  return (
    <Frame title={spec.title} xLabel={spec.xLabel} yLabel={spec.yLabel}>
      ...
      <line
        ...
        y1={top.y}
        y2={top.y + top.height}
        stroke={chartTheme.lavender}
        strokeDasharray="5 5"
      />
      <text
        x={populationMean + 6}
        y={top.y + 14}
        fill={chartTheme.lavender}
        fontSize={11}
        fontWeight={800}
      >
        μ
      ...
      {x.ticks(5).map((tick) => (
        <g key={`mean-tick-${tick}`}>
          <line
            ...
            x2={x(tick)}
            y1={bottom.y + bottom.height}
            y2={bottom.y + bottom.height + 5}
            stroke={chartTheme.axis}
          />
          <text
            x={x(tick)}
            y={bottom.y + bottom.height + 18}
            textAnchor="middle"
            fill={chartTheme.muted}
            fontSize={10}
          >
...
function AnovaChart({ spec }: { spec: Extract<ChartSpec, { type: "anova" }> }) {
  ...
  const y = scaleLinear()
    ...
    .range([height - margin.bottom, margin.top]);
  const x = scaleBand()
    .domain(spec.groups.map((group) => group.label))
    .range([margin.left, width - margin.right])
    .padding(0.24);
  return (
    <Frame title={spec.title} xLabel={spec.xLabel} yLabel={spec.yLabel}>
      ...
      <text
        ...
        y={y(spec.grandMean) - 7}
        textAnchor="end"
        fill={chartTheme.lavender}
        fontSize={10.5}
        fontWeight={800}
      >
        {spec.grandMeanLabel ?? "grand mean"}
      </text>
        ...
        return (
          <g key={group.label}>
            {group.values.map((value, valueIndex) => {
              const jitter =
                (((valueIndex * 37 + groupIndex * 19) % 17) / 16 - 0.5) * x.bandwidth() * 0.58;
              return (
                <circle
                  key={`${group.label}-${valueIndex}`}
                  cx={center + jitter}
                  cy={y(value)}
                  r={4.4}
                  fill={chartTheme.teal}
                  opacity={0.7}
                >
                  <title>{`${group.label}: ${value.toFixed(3)}`}</title>
                </circle>
              );
            })}
            <line
              x1={start + x.bandwidth() * 0.15}
              x2={start + x.bandwidth() * 0.85}
              y1={y(group.mean)}
              y2={y(group.mean)}
      ...
      <Legend
        items={[
          ...
          {
            label: spec.grandMeanLabel ?? "grand mean",
            color: chartTheme.lavender,
            shape: "dashed",
          },
        ]}
      />
...
function McmcChart({ spec }: { spec: Extract<ChartSpec, { type: "mcmc" }> }) {
  ...
  const x = scaleLinear()
    .domain(spec.xDomain)
    .range([left.x, left.x + left.width]);
  const y = scaleLinear()
    .domain(spec.yDomain)
    .range([left.y + left.height, left.y]);
  const traceValues = [...spec.traceX, ...spec.traceY].map((point) => point.y);
  const traceDomain = extent(traceValues, spec.yDomain);
  const traceXDomain = extent(
    spec.traceX.map((point) => point.x),
    [0, 1],
  );
  const traceX = scaleLinear()
    .domain(traceXDomain)
    .range([right.x, right.x + right.width]);
  const traceYTop = scaleLinear()
    .domain(traceDomain)
    .nice()
    .range([right.y + right.height, right.y]);
  const traceYBottom = scaleLinear()
    .domain(traceDomain)
    .nice()
    .range([rightBottom.y + rightBottom.height, rightBottom.y]);
  const targetLine = line<ChartPoint>()
    .x((point) => x(point.x))
    .y((point) => y(point.y));
  const traceLineTop = line<ChartPoint>()
    .x((point) => traceX(point.x))
    .y((point) => traceYTop(point.y));
  const traceLineBottom = line<ChartPoint>()
    .x((point) => traceX(point.x))
    .y((point) => traceYBottom(point.y));
  const pathLine = line<ChartPoint>()
    .x((point) => x(point.x))
    .y((point) => y(point.y));
  ...
  return (
    <Frame title={spec.title} xLabel="" yLabel="">
      <text x={left.x} y={left.y - 14} fill={chartTheme.text} fontSize={12} fontWeight={800}>
        {spec.targetLabel}
      </text>
      <rect
        x={left.x}
      ...
      {spec.contours.map((contour, index) => (
        <path
          key={`mcmc-contour-${index}`}
          d={targetLine(contour.points) ?? ""}
          fill="none"
          stroke={contour.color ?? chartTheme.lavender}
          strokeWidth={1.4}
          strokeOpacity={contour.opacity ?? 0.55}
        />
      ))}
      {spec.samples.map((point, index) => (
        <circle
          key={`mcmc-sample-${index}`}
          cx={x(point.x)}
          cy={y(point.y)}
          r={2.3}
          fill={point.color ?? chartTheme.teal}
      ...
      {spec.path.map((point, index) => (
        <circle
          ...
          cx={x(point.x)}
          cy={y(point.y)}
          r={index === spec.path.length - 1 ? 5 : 2.5}
          fill={index === spec.path.length - 1 ? chartTheme.danger : chartTheme.lavender}
        />
      ))}
      {latest && (
        <text
          ...
          fontWeight={800}
        >
          {spec.currentStateLabel ?? "current state"}
        </text>
      )}
      <text
        x={left.x + left.width / 2}
        y={left.y + left.height + 26}
        textAnchor="middle"
        fill={chartTheme.muted}
        fontSize={11}
      >
      ...
      <text
        ...
        fill={chartTheme.muted}
        fontSize={11}
        transform={`rotate(-90 ${left.x - 34} ${left.y + left.height / 2})`}
      >
        {spec.yLabel}
      </text>

      <text x={right.x} y={right.y - 14} fill={chartTheme.text} fontSize={12} fontWeight={800}>
        {spec.traceLabel}
      </text>
      <rect
        x={right.x}
      ...
      />
      <path
        d={traceLineTop(spec.traceX) ?? ""}
        fill="none"
        stroke={chartTheme.teal}
        strokeWidth={2}
      />
      <text x={right.x + 8} y={right.y + 15} fill={chartTheme.teal} fontSize={10} fontWeight={800}>
        x₁
      </text>
      <rect
        x={rightBottom.x}
      ...
      <Legend
        items={[
          {
            label: spec.targetDensityLabel ?? "target density",
            color: chartTheme.lavender,
            shape: "line",
          },
