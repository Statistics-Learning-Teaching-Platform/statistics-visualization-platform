...
function BarsChart({ spec }: { spec: Extract<ChartSpec, { type: "bars" }> }) {
  ...
  return (
    <Frame title={spec.title} xLabel={spec.xLabel} yLabel={spec.yLabel}>
      ...
      {spec.bars.map((bar, i) => (
        <g key={i}>
          <rect x={x(i) ?? margin.left} y={y(bar.value)} width={x.bandwidth()} height={Math.max(0, height - margin.bottom - y(bar.value))} fill={bar.semanticColor ?? bar.color ?? chartTheme.teal} opacity={0.86}>
