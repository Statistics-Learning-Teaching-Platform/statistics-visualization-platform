...
const DYNAMIC_TEMPLATES: DynamicTemplate[] = [
  ...
  { pattern: /^(\d+) observed values \(input limit applied\)$/, translate: (m) => `${m[1]} 个观测值（已应用输入上限）` },
  { pattern: /^(\d+) requested; ([\d,]+) total draws$/, translate: (m) => `请求 ${m[1]} 次重复；实际共 ${m[2]} 次抽样` },
  { pattern: /^(\d+) between-mode transitions$/, translate: (m) => `${m[1]} 次模态间转移` },
  { pattern: /^target density ([\d.]+) mode (\d+)$/, translate: (m) => `目标密度 ${m[1]}，模态 ${m[2]}` },
