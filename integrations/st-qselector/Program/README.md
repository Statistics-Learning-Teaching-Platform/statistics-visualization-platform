...
默认 AI 地址指向项目约定的服务，也可用 `STAT_AI_API_URL` 覆盖。模型列表不写入配置或浏览器存储：用户点击扫描按钮时实时读取上游列表，只有上游报告为已加载的模型可以选择，其余统一显示为“未启用”。每次生成及重试前，服务端会再次确认所选模型仍处于已加载状态；模型被卸载或移除时要求重新扫描，不自动换用其他模型。所有 AI 路由都要求登录；知识提取和组卷仅允许教师或超级管理员调用。
