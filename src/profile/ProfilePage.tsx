...
const copy = {
	...
	en: {
		...
		evidenceIntro:
			"Only progress actually stored by the product is shown here.",
		pathEyebrow: "RECOMMENDED FLOW",
...
function getResumeDestination(route: string, language: Language) {
	...
	const href = allowedPrefixes.some((prefix) => route.startsWith(prefix))
		? route
		: "/catalog";
	const labels =
		language === "zh"
						...
						{spaces.map((space) => {
							...
							return (
								...
								>
									...
									<span className="ed-profile-space__status">
										{space.status}
									</span>
