...
import { lessonUnits, rLessons, type RLesson } from "./lessons";
...
const PROGRESS_KEY = "statmind-r-learning-progress-v1";
...
function TutorAnswer({ content }: { content: string }) {
  const parts = content.split(/```(?:r)?\s*([\s\S]*?)```/gi);
  return (
...
export function RLearningWorkspace() {
  ...
  const [activeId, setActiveId] = useState(rLessons[0].id);
  ...
  async function askTutor(suggestedQuestion?: string) {
    ...
    try {
      ...
      setTutorMessages((current) => [
        ...current,
        { id: crypto.randomUUID(), role: "assistant", content: payload.answer as string },
      ]);
      setTutorStatus("idle");
    } catch {
      setTutorMessages((current) => [
        ...current,
        { id: crypto.randomUUID(), role: "assistant", content: t.tutorError },
      ]);
    ...
    <main className="r-learning-shell">
      <header className="r-learning-header">
        ...
        <div className="r-learning-progress" aria-label={`${t.progress}: ${progress}%`}>
          <span><b>{completed.length}</b> / {rLessons.length} {t.complete}</span>
      ...
      <div className="r-learning-layout">
        <aside className="r-lesson-sidebar">
          <div className="r-sidebar-heading">
            <span>{t.lessons}</span>
            <b>{String(rLessons.length).padStart(2, "0")}</b>
          </div>
          <nav className="r-lesson-nav" aria-label={t.lessons}>
            {lessonUnits.map((unit) => {
              const unitLessons = rLessons.filter((lesson) => lesson.unit === unit.id);
          ...
          <div className="r-assistance-grid">
            ...
            <section className="r-assistance-card">
              <button type="button" onClick={() => setShowSolution((value) => !value)}>
                <span>R</span><strong>{t.solution}</strong><b>{showSolution ? "−" : "+"}</b>
              </button>
        ...
        <aside ref={outputSidebarRef} className="r-output-sidebar">
          ...
          <div className="r-output-heading"><strong>{t.runtime}</strong><small>WebR · local</small></div>
          <div className="r-output-tabs" role="tablist" aria-label="R output">
          ...
          <section className="r-output-panel" role="tabpanel">
            ...
            {outputTab === "plot" && (
              plot ? <CanvasPlot image={plot} /> : <div className="r-empty-output"><span className="r-empty-output__plot">⌁</span><p>{t.noPlot}</p></div>
            )}
            {outputTab === "environment" && (
              environment.length ? (
                <ul className="r-environment-list">
                  {environment.map((object) => <li key={object}><span>R</span><code>{object}</code></li>)}
                </ul>
            ...
            {outputTab === "review" && (
              review.kind === "idle" ? (
                ...
                <div className="r-review-result" data-kind={review.kind}>
                  ...
                  <p>{review.message}</p>
                  {review.kind === "success" && activeLesson.order < rLessons.length ? (
                    <button type="button" onClick={() => selectLesson(rLessons[activeLesson.order].id)}>{t.next} →</button>
                  ) : null}
                </div>
              )
            )}
