...
confidence_interval`,
  ...
  },
  {
    ...
    objective: { zh: "使用 `summary()` 和 `boxplot()` 观察中心、四分位数与潜在离群值。", en: "Use `summary()` and `boxplot()` to inspect centre, quartiles, and potential outliers." },
    explanation: { zh: "箱线图把中位数、四分位距和离群观测放在同一图形中；它比只看均值更能揭示偏斜和极端值。", en: "A boxplot shows the median, IQR, and outliers together, often revealing skew and extremes that a mean alone hides." },
    task: { zh: "生成五数概括 `summary_stats`，并用 `boxplot()` 保存图形对象 `boxplot_result`。", en: "Create a five-number summary as `summary_stats` and save the boxplot object as `boxplot_result`." },
...
summary_stats`,
    hint: { zh: "`summary(wait)` 返回最小值、四分位数、中位数、均值和最大值；`boxplot(wait, plot = FALSE)` 可保存统计结果。", en: "Use `summary(wait)` for the five-number summary and `boxplot(wait, plot = FALSE)` to store boxplot statistics." },
...
summary_stats`,
    ...
    success: { zh: "完成：你已用五数概括和箱线图识别出极端等待时间。", en: "Complete: the five-number summary and boxplot reveal the extreme waiting time." },
  },
  {
    language: "r",
    ...
    objective: { zh: "使用 `qqnorm()` 与 `qqline()` 比较样本分位数和正态理论分位数。", en: "Use `qqnorm()` and `qqline()` to compare sample and theoretical normal quantiles." },
    explanation: { zh: "如果点大致沿直线排列，正态分布是一个合理的近似；尾部弯曲通常提示偏态或重尾。", en: "Points close to a straight line support a normal approximation; systematic tail curvature suggests skew or heavy tails." },
    task: { zh: "对 `scores` 生成 QQ 图，保存点和参考线的返回对象。", en: "Create a QQ plot for `scores` and save the point and reference-line results." },
...
qq_line <-
qq_points`,
    hint: { zh: "使用 `qqnorm(scores, plot.it = FALSE)` 得到点，再用 `qqline(scores, plot.it = FALSE)` 得到参考线参数。", en: "Use `qqnorm(scores, plot.it = FALSE)` for points and `qqline(scores, plot.it = FALSE)` for the reference line." },
...
qq_points`,
    checkCode: `exists("qq_points") && is.list(qq_points) &&
      ...
      length(qq_line$y) == 2`,
    success: { zh: "完成：QQ 图的理论分位数和样本分位数已准备好比较。", en: "Complete: the theoretical and sample quantiles are ready to compare." },
  },
  {
    language: "r",
    ...
    objective: { zh: "使用独立样本 t 检验比较两组均值，并提取差异的置信区间。", en: "Use an independent-samples t test and extract the confidence interval for the difference." },
    explanation: { zh: "两组 t 检验考察均值差是否足以超出抽样变异；p 值需要结合效应大小和区间解释。", en: "A two-sample t test asks whether the mean difference is large relative to sampling variation; interpret p-values with effect size and intervals." },
    task: { zh: "比较两种教学方法的成绩，将结果保存为 `test_result`，均值差保存为 `mean_difference`。", en: "Compare scores from two teaching methods and save the test as `test_result` and the mean difference as `mean_difference`." },
...
test_result`,
    hint: { zh: "使用 `t.test(method_b, method_a)`；均值差可以用 `mean(method_b) - mean(method_a)`。", en: "Use `t.test(method_b, method_a)` and compute `mean(method_b) - mean(method_a)`." },
...
test_result`,
    ...
    success: { zh: "完成：两组均值差、t 检验和差异区间均已计算。", en: "Complete: the mean difference, t test, and interval for the difference are available." },
  },
  {
    language: "r",
    ...
    objective: { zh: "使用 `aov()` 和 `summary()` 检查三组均值是否存在整体差异。", en: "Use `aov()` and `summary()` to test for an overall difference among three means." },
    explanation: { zh: "ANOVA 把总变异拆成组间和组内两部分；显著的 F 检验只说明至少有一组不同，不能直接告诉你是哪一组。", en: "ANOVA decomposes total variation into between- and within-group components; a significant F test says at least one group differs, not which one." },
    task: { zh: "拟合 `score ~ group` 的模型，将模型保存为 `anova_model`，并提取 p 值 `anova_p`。", en: "Fit `score ~ group`, save it as `anova_model`, and extract the p-value as `anova_p`." },
...
summary(anova_model)`,
    hint: { zh: '先用 `aov(score ~ group)`，再从 `summary(anova_model)[[1]][["Pr(>F)"]][1]` 取 p 值。', en: 'Fit `aov(score ~ group)` and read the first p-value from `summary(anova_model)[[1]][["Pr(>F)"]][1]`.' },
...
summary(anova_model)`,
    ...
    success: { zh: "完成：整体 F 检验显示三组均值并不完全相同。", en: "Complete: the overall F test indicates that the three group means are not all equal." },
  },
  {
    language: "r",
    ...
    objective: { zh: "使用 `cor.test()` 同时获得相关系数和线性相关的 p 值。", en: "Use `cor.test()` to obtain a correlation coefficient and a p-value for linear association." },
    explanation: { zh: "相关描述线性关联，不等于因果关系；散点图、研究设计和变量单位同样重要。", en: "Correlation describes linear association, not causation; inspect the scatterplot, design, and units as well." },
    task: { zh: "检验学习时间和成绩的 Pearson 相关，将检验结果保存为 `correlation_result`，相关系数保存为 `r_value`。", en: "Test the Pearson correlation between study time and score and save the result and coefficient." },
...
correlation_result`,
    hint: { zh: '使用 `cor.test(hours, score, method = "pearson")`，再读取 `$estimate`。', en: 'Use `cor.test(hours, score, method = "pearson")` and read `$estimate`.' },
...
correlation_result`,
    ...
    success: { zh: "完成：相关系数与相关检验结果一致；请记住这不是因果证明。", en: "Complete: the coefficient and test agree; remember that this is not evidence of causation." },
  },
  {
    language: "r",
    prerequisites: ["data-frame-filter"],
    ...
    objective: { zh: "使用 `chisq.test()` 比较观察频数和独立性假设下的期望频数。", en: "Use `chisq.test()` to compare observed counts with expected counts under independence." },
    explanation: { zh: "卡方独立性检验针对分类变量之间的关联；小期望频数时需要合并类别或采用精确方法。", en: "A chi-square independence test examines association between categorical variables; small expected counts require care." },
    task: { zh: "对学习方式和是否通过考试的列联表进行检验，保存结果和期望频数。", en: "Test the association between study mode and passing, saving the result and expected counts." },
...
chi_result`,
    hint: { zh: "运行 `chisq.test(counts, correct = FALSE)`，期望频数在 `$expected`。", en: "Run `chisq.test(counts, correct = FALSE)` and read expected counts from `$expected`." },
...
chi_result`,
    ...
    success: { zh: "完成：观察频数、期望频数和独立性检验结果均已得到。", en: "Complete: observed counts, expected counts, and the independence test are available." },
  },
  {
    language: "r",
    ...
    objective: { zh: "用 `filter()` 计算三期移动平均，并用 MAE 比较预测与观测。", en: "Use `filter()` for a three-period moving average and evaluate it with MAE." },
    explanation: { zh: "移动平均平滑短期波动，但会牺牲响应速度；预测误差应在同一尺度上解释。", en: "A moving average smooths short-term noise at the cost of responsiveness; interpret errors on the original scale." },
    task: { zh: "计算居中的三期移动平均 `moving_average`，并把非缺失预测与观测的平均绝对误差保存为 `mae`。", en: "Compute a centred three-period moving average and save the MAE for non-missing predictions." },
...
mae`,
    hint: { zh: "使用 `stats::filter(sales, rep(1/3, 3), sides = 2)`；MAE 是 `mean(abs(sales[valid] - moving_average[valid]))`。", en: "Use `stats::filter(sales, rep(1/3, 3), sides = 2)` and average the absolute errors where the forecast is not missing." },
...
mae`,
    ...
    success: { zh: "完成：三期移动平均和 MAE 已计算，可继续比较其他预测方法。", en: "Complete: the three-period moving average and MAE are ready for comparison." },
  },
  {
    language: "r", prerequisites: ["vectors-and-mean"], id: "r-ecosystem", topicId: "statistics-foundations", unit: "foundations", order: 15,
    eyebrow: { zh: "第十五课 · R 环境", en: "Lesson 15 · The R ecosystem" }, title: { zh: "认识 RStudio 与可复现脚本", en: "Work with RStudio and reproducible scripts" },
    objective: { zh: "理解脚本、工作区和包的关系，建立可复现的分析习惯。", en: "Understand scripts, workspaces, and packages as the basis of reproducible analysis." }, explanation: { zh: "把分析步骤写进脚本，避免只依赖控制台历史。", en: "Keep analysis steps in a script instead of relying on console history." }, task: { zh: "记录分析名称和当前 R 版本。", en: "Record an analysis name and the current R version." }, concepts: ["RStudio", "script", "sessionInfo()"],
    starterCode: "analysis_name <- \\\"descriptive statistics\\\"\\nr_version <- ", hint: { zh: "使用 `R.version.string`。", en: "Use `R.version.string`." }, solution: "analysis_name <- \\\"descriptive statistics\\\"\\nr_version <- R.version.string", checkCode: "exists(\\\"analysis_name\\\") && is.character(analysis_name) && exists(\\\"r_version\\\") && is.character(r_version)", success: { zh: "完成：运行环境已记录。", en: "Complete: the runtime environment is recorded." },
  },
  {
    language: "r", prerequisites: ["r-ecosystem"], id: "r-data-import", topicId: "data-and-variables", unit: "data", order: 16,
    eyebrow: { zh: "第十六课 · 导入数据", en: "Lesson 16 · Import data" }, title: { zh: "从文本表格建立数据框", en: "Build a data frame from tabular text" }, objective: { zh: "理解列名、分隔符和导入后的数据结构。", en: "Understand headers, separators, and the imported structure." }, explanation: { zh: "导入设置会影响列类型、缺失值和后续分析。", en: "Import settings affect column types, missing values, and later analyses." }, task: { zh: "把文本读成 `survey`，保存行数和列数。", en: "Read the text into `survey` and save row and column counts." }, concepts: ["read.csv()", "header", "str()"],
    starterCode: "raw <- \\\"id,group,score\\\\n1,A,82\\\\n2,B,76\\\\n3,A,91\\\"\\nsurvey <- \\nn_rows <- \\nn_cols <- ", hint: { zh: "使用 `read.csv(text = raw, header = TRUE)`。", en: "Use `read.csv(text = raw, header = TRUE)`." }, solution: "raw <- \\\"id,group,score\\\\n1,A,82\\\\n2,B,76\\\\n3,A,91\\\"\\nsurvey <- read.csv(text = raw, header = TRUE)\\nn_rows <- nrow(survey)\\nn_cols <- ncol(survey)", checkCode: "is.data.frame(survey) && n_rows == 3 && n_cols == 3", success: { zh: "完成：数据结构检查通过。", en: "Complete: the data structure checks out." },
  },
  {
    language: "r", prerequisites: ["data-frame-filter"], id: "r-stem-leaf", topicId: "visual-encoding", unit: "data", order: 17,
    eyebrow: { zh: "第十七课 · 茎叶图", en: "Lesson 17 · Stem-and-leaf plots" }, title: { zh: "用茎叶图观察分布", en: "Inspect a distribution with a stem-and-leaf plot" }, objective: { zh: "使用 `stem()` 查看分布形状。", en: "Use `stem()` to inspect distribution shape." }, explanation: { zh: "茎叶图保留原始数值，同时提供紧凑的分布概览。", en: "A stem-and-leaf plot keeps original values while showing shape compactly." }, task: { zh: "保存茎叶图输出并计算中位数。", en: "Save the display and calculate the median." }, concepts: ["stem()", "median()"],
    starterCode: "scores <- c(61, 64, 67, 72, 74, 75, 81, 88, 90)\\nstem_display <- \\nmedian_score <- ", hint: { zh: "使用 `capture.output(stem(scores))` 和 `median(scores)`。", en: "Use `capture.output(stem(scores))` and `median(scores)`." }, solution: "scores <- c(61, 64, 67, 72, 74, 75, 81, 88, 90)\\nstem_display <- capture.output(stem(scores))\\nmedian_score <- median(scores)", checkCode: "length(stem_display) > 0 && median_score == 74", success: { zh: "完成：分布输出与中心统计量已保存。", en: "Complete: the display and centre statistic are saved." },
  },
  {
    language: "r", prerequisites: ["r-stem-leaf"], id: "r-barplot-categorical", topicId: "visual-encoding", unit: "data", order: 18,
    eyebrow: { zh: "第十八课 · 分类图形", en: "Lesson 18 · Categorical graphics" }, title: { zh: "绘制分类频数柱状图", en: "Plot categorical frequencies" }, objective: { zh: "用 `table()` 汇总分类变量，再用 `barplot()` 表达频数。", en: "Summarize categories with `table()` and display frequencies with `barplot()`." }, explanation: { zh: "柱高表示类别频数，类别顺序应明确。", en: "Bar heights represent category counts and category order should be explicit." }, task: { zh: "保存频数表和柱高。", en: "Save the frequency table and bar heights." }, concepts: ["table()", "barplot()"],
    starterCode: "device <- c(\\\"web\\\", \\\"mobile\\\", \\\"web\\\", \\\"tablet\\\", \\\"mobile\\\", \\\"web\\\")\\ndevice_counts <- \\nbar_heights <- ", hint: { zh: "使用 `table(device)` 和 `barplot(..., plot = FALSE)`。", en: "Use `table(device)` and `barplot(..., plot = FALSE)`." }, solution: "device <- c(\\\"web\\\", \\\"mobile\\\", \\\"web\\\", \\\"tablet\\\", \\\"mobile\\\", \\\"web\\\")\\ndevice_counts <- table(device)\\nbar_heights <- barplot(device_counts, plot = FALSE)", checkCode: "identical(as.integer(device_counts), c(1L, 2L, 3L)) && length(bar_heights) == 3", success: { zh: "完成：频数汇总与图形编码一致。", en: "Complete: the summary and graphical encoding agree." },
  },
  {
    language: "r", prerequisites: ["r-barplot-categorical"], id: "r-grouped-barplot", topicId: "anova", unit: "data", order: 19,
    eyebrow: { zh: "第十九课 · 分组比较", en: "Lesson 19 · Group comparisons" }, title: { zh: "比较不同组的平均值", en: "Compare group means" }, objective: { zh: "用 `tapply()` 汇总两因素数据。", en: "Summarize two-factor data with `tapply()`." }, explanation: { zh: "分组均值是描述性统计，不等同于显著性检验。", en: "Group means are descriptive and are not a significance test." }, task: { zh: "计算教学方式和班级的均值矩阵。", en: "Compute a mean matrix for teaching method by class." }, concepts: ["tapply()", "grouped summary"],
    starterCode: "method <- c(\\\"video\\\", \\\"video\\\", \\\"text\\\", \\\"text\\\", \\\"video\\\", \\\"text\\\")\\nclass <- c(\\\"A\\\", \\\"B\\\", \\\"A\\\", \\\"B\\\", \\\"A\\\", \\\"B\\\")\\nscore <- c(78, 84, 72, 76, 81, 79)\\nmean_table <- ", hint: { zh: "使用 `tapply(score, list(method, class), mean)`。", en: "Use `tapply(score, list(method, class), mean)`." }, solution: "method <- c(\\\"video\\\", \\\"video\\\", \\\"text\\\", \\\"text\\\", \\\"video\\\", \\\"text\\\")\\nclass <- c(\\\"A\\\", \\\"B\\\", \\\"A\\\", \\\"B\\\", \\\"A\\\", \\\"B\\\")\\nscore <- c(78, 84, 72, 76, 81, 79)\\nmean_table <- tapply(score, list(method, class), mean)", checkCode: "is.matrix(mean_table) && all(dim(mean_table) == c(2, 2))", success: { zh: "完成：分组均值矩阵已建立。", en: "Complete: the group-mean matrix is ready." },
  },
  { language: "r", prerequisites: ["mean-confidence-interval"], id: "r-t-interval-manual", topicId: "confidence-interval", unit: "statistics", order: 20, eyebrow: { zh: "第二十课 · 手算置信区间", en: "Lesson 20 · Manual confidence intervals" }, title: { zh: "用 t 分布手动构造均值区间", en: "Construct a mean interval with the t distribution" }, objective: { zh: "把标准误、临界值和误差界限连接起来。", en: "Connect the standard error, critical value, and margin of error." }, explanation: { zh: "区间等于样本均值加减临界值乘以标准误。", en: "An interval is the sample mean plus or minus a critical value times the standard error." }, task: { zh: "保存下限、上限和自由度。", en: "Save the lower bound, upper bound, and degrees of freedom." }, concepts: ["qt()", "standard error", "margin of error"], starterCode: "sample <- c(12, 15, 13, 16, 14, 11, 15, 14)\\nconfidence <- 0.95\\nmean_value <- mean(sample)\\nse <- sd(sample) / sqrt(length(sample))\\ndf <- length(sample) - 1\\ncritical <-\\nlower <-\\nupper <-", hint: { zh: "使用 `qt((1 + confidence) / 2, df)`，再计算上下界。", en: "Use `qt((1 + confidence) / 2, df)` and compute both bounds." }, solution: "sample <- c(12, 15, 13, 16, 14, 11, 15, 14)\\nconfidence <- 0.95\\nmean_value <- mean(sample)\\nse <- sd(sample) / sqrt(length(sample))\\ndf <- length(sample) - 1\\ncritical <- qt((1 + confidence) / 2, df)\\nlower <- mean_value - critical * se\\nupper <- mean_value + critical * se", checkCode: "df == 7 && lower < mean_value && upper > mean_value", success: { zh: "完成：t 临界值、标准误和区间边界已经对应起来。", en: "Complete: the t critical value, standard error, and bounds are connected." } },
  { language: "r", prerequisites: ["two-sample-t-test"], id: "r-paired-t-test", topicId: "hypothesis-testing", unit: "statistics", order: 21, eyebrow: { zh: "第二十一课 · 配对检验", en: "Lesson 21 · Paired tests" }, title: { zh: "检验前后测量的平均变化", en: "Test mean change in paired measurements" }, objective: { zh: "理解配对设计应先计算每一对的差值。", en: "Understand that a paired design is analysed through within-pair differences." }, explanation: { zh: "配对 t 检验检验的是差值的平均值。", en: "A paired t test targets the mean of within-pair changes." }, task: { zh: "保存差值向量和配对检验结果。", en: "Save the change vector and paired test result." }, concepts: ["paired t-test", "difference scores", "t.test()"], starterCode: "before <- c(68, 72, 75, 70, 66, 74)\\nafter <- c(72, 75, 79, 73, 70, 78)\\nchange <-\\npaired_result <-", hint: { zh: "先用 `after - before`，再运行配对 `t.test()`。", en: "Compute `after - before`, then run paired `t.test()`." }, solution: "before <- c(68, 72, 75, 70, 66, 74)\\nafter <- c(72, 75, 79, 73, 70, 78)\\nchange <- after - before\\npaired_result <- t.test(after, before, paired = TRUE)", checkCode: "length(change) == 6 && all(change > 0) && inherits(paired_result, \\\"htest\\\")", success: { zh: "完成：检验对象是每个个体的前后差值。", en: "Complete: the test targets each participant's before-after difference." } },
  { language: "r", prerequisites: ["linear-regression"], id: "r-multiple-regression", topicId: "linear-regression", unit: "statistics", order: 22, eyebrow: { zh: "第二十二课 · 多元回归", en: "Lesson 22 · Multiple regression" }, title: { zh: "控制另一个变量后解释成绩", en: "Explain scores while controlling for another variable" }, objective: { zh: "拟合含两个解释变量的线性模型。", en: "Fit a linear model with two predictors." }, explanation: { zh: "每个系数表示控制其他变量后的平均关联，不自动意味着因果。", en: "Each coefficient is an adjusted association, not automatically causal." }, task: { zh: "提取系数、拟合值和 R²。", en: "Extract coefficients, fitted values, and R-squared." }, concepts: ["lm()", "multiple regression", "R-squared"], starterCode: "hours <- c(1, 2, 3, 4, 5, 6)\\nattendance <- c(70, 72, 75, 79, 82, 86)\\nscore <- c(55, 59, 65, 70, 77, 84)\\nmodel <- lm(score ~ hours + attendance)\\ncoefficients <-\\nfitted <-\\nr_squared <-", hint: { zh: "从 `summary(model)` 和 `fitted(model)` 提取。", en: "Extract from `summary(model)` and `fitted(model)`." }, solution: "hours <- c(1, 2, 3, 4, 5, 6)\\nattendance <- c(70, 72, 75, 79, 82, 86)\\nscore <- c(55, 59, 65, 70, 77, 84)\\nmodel <- lm(score ~ hours + attendance)\\ncoefficients <- summary(model)$coefficients\\nfitted <- fitted(model)\\nr_squared <- summary(model)$r.squared", checkCode: "inherits(model, \\\"lm\\\") && nrow(coefficients) == 3 && length(fitted) == 6 && r_squared > 0.9", success: { zh: "完成：多元模型的系数、拟合值和解释度已提取。", en: "Complete: coefficients, fitted values, and explained variance are extracted." } },
  { language: "r", prerequisites: ["r-multiple-regression"], id: "r-regression-anova", topicId: "linear-regression", unit: "statistics", order: 23, eyebrow: { zh: "第二十三课 · 回归方差分解", en: "Lesson 23 · Regression ANOVA" }, title: { zh: "用 ANOVA 表检查回归模型", en: "Inspect a regression model with ANOVA" }, objective: { zh: "理解回归平方和、残差平方和与 F 检验。", en: "Understand regression and residual sums of squares and the F test." }, explanation: { zh: "ANOVA 表把总变异分成模型和残差两部分。", en: "The ANOVA table partitions total variation into model and residual parts." }, task: { zh: "保存 ANOVA 表和整体模型 p 值。", en: "Save the ANOVA table and overall model p-value." }, concepts: ["anova()", "F statistic", "residual variation"], starterCode: "x <- c(1, 2, 3, 4, 5, 6, 7)\\ny <- c(52, 56, 60, 63, 69, 73, 78)\\nmodel <- lm(y ~ x)\\nanova_table <-\\nmodel_p <-", hint: { zh: "使用 `anova(model)` 和 `pf()`。", en: "Use `anova(model)` and `pf()`." }, solution: "x <- c(1, 2, 3, 4, 5, 6, 7)\\ny <- c(52, 56, 60, 63, 69, 73, 78)\\nmodel <- lm(y ~ x)\\nanova_table <- anova(model)\\nf <- summary(model)$fstatistic\\nmodel_p <- pf(f[1], f[2], f[3], lower.tail = FALSE)", checkCode: "is.data.frame(anova_table) && nrow(anova_table) == 2 && model_p < 0.01", success: { zh: "完成：模型与残差变异已经在 ANOVA 表中分开。", en: "Complete: model and residual variation are separated in the ANOVA table." } },
  { language: "r", prerequisites: ["r-regression-anova"], id: "r-stepwise-selection", topicId: "linear-regression", unit: "statistics", order: 24, eyebrow: { zh: "第二十四课 · 变量选择", en: "Lesson 24 · Variable selection" }, title: { zh: "比较逐步回归模型", en: "Compare stepwise regression models" }, objective: { zh: "用 AIC 比较嵌套模型，并明确选择不确定性。", en: "Compare nested models with AIC while recognising selection uncertainty." }, explanation: { zh: "逐步选择是探索工具，不应替代领域知识或验证。", en: "Stepwise selection is exploratory and should not replace subject knowledge or validation." }, task: { zh: "保存完整模型和 AIC 更小的模型。", en: "Save a full model and the lower-AIC selected model." }, concepts: ["AIC", "step()", "model selection"], starterCode: "data <- data.frame(y = c(52, 56, 60, 63, 69, 73, 78), x1 = 1:7, x2 = c(3, 2, 4, 3, 5, 4, 6))\\nfull_model <- lm(y ~ x1 + x2, data = data)\\nselected_model <-", hint: { zh: "使用 `step(full_model, direction = \"backward\", trace = 0)`。", en: "Use `step(full_model, direction = \"backward\", trace = 0)`." }, solution: "data <- data.frame(y = c(52, 56, 60, 63, 69, 73, 78), x1 = 1:7, x2 = c(3, 2, 4, 3, 5, 4, 6))\\nfull_model <- lm(y ~ x1 + x2, data = data)\\nselected_model <- step(full_model, direction = \"backward\", trace = 0)", checkCode: "inherits(full_model, \\\"lm\\\") && inherits(selected_model, \\\"lm\\\") && AIC(selected_model) <= AIC(full_model) + 1e-8", success: { zh: "完成：模型选择结果已记录，但仍需验证。", en: "Complete: the selected model is recorded but still needs validation." } },
  { language: "r", prerequisites: ["r-stepwise-selection"], id: "r-stepwise-anova", topicId: "anova", unit: "statistics", order: 25, eyebrow: { zh: "第二十五课 · 模型比较", en: "Lesson 25 · Model comparison" }, title: { zh: "用 ANOVA 比较嵌套模型", en: "Compare nested models with ANOVA" }, objective: { zh: "用额外平方和检验判断加入变量是否带来证据。", en: "Use an extra-sum-of-squares test to assess evidence for an added predictor." }, explanation: { zh: "嵌套模型比较检验额外变量解释的残差减少，不是因果作用。", en: "Nested-model comparison tests residual reduction, not causality." }, task: { zh: "保存简化模型、完整模型和比较结果。", en: "Save reduced and full models and their comparison." }, concepts: ["anova()", "nested models"], starterCode: "data <- data.frame(y = c(52, 56, 60, 63, 69, 73, 78), x1 = 1:7, x2 = c(3, 2, 4, 3, 5, 4, 6))\\nreduced <- lm(y ~ x1, data = data)\\nfull <- lm(y ~ x1 + x2, data = data)\\ncomparison <-", hint: { zh: "使用 `anova(reduced, full)`。", en: "Use `anova(reduced, full)`." }, solution: "data <- data.frame(y = c(52, 56, 60, 63, 69, 73, 78), x1 = 1:7, x2 = c(3, 2, 4, 3, 5, 4, 6))\\nreduced <- lm(y ~ x1, data = data)\\nfull <- lm(y ~ x1 + x2, data = data)\\ncomparison <- anova(reduced, full)", checkCode: "is.data.frame(comparison) && nrow(comparison) == 2", success: { zh: "完成：嵌套模型的额外解释量已经比较。", en: "Complete: the added explanatory contribution is compared." } },
  { language: "r", prerequisites: ["probability-foundations"], id: "r-probability-events", topicId: "probability-foundations", unit: "statistics", order: 26, eyebrow: { zh: "第二十六课 · 条件概率", en: "Lesson 26 · Conditional probability" }, title: { zh: "用贝叶斯公式更新概率", en: "Update probabilities with Bayes' rule" }, objective: { zh: "区分联合概率、条件概率和后验概率。", en: "Distinguish joint, conditional, and posterior probabilities." }, explanation: { zh: "贝叶斯公式把先验信息与新证据结合。", en: "Bayes' rule combines prior information with evidence." }, task: { zh: "保存阳性检测后的后验概率 `posterior`。", en: "Save the posterior probability as `posterior`." }, concepts: ["conditional probability", "Bayes rule"], starterCode: "prior <- 0.01\\nsensitivity <- 0.95\\nfalse_positive <- 0.05\\nposterior <- ", hint: { zh: "使用贝叶斯公式。", en: "Use Bayes' formula." }, solution: "prior <- 0.01\\nsensitivity <- 0.95\\nfalse_positive <- 0.05\\nposterior <- prior * sensitivity / (prior * sensitivity + (1 - prior) * false_positive)", checkCode: "posterior > 0 && posterior < 1", success: { zh: "完成：后验概率同时受到患病率和检测性能影响。", en: "Complete: the posterior depends on prevalence and test performance." } },
  { language: "r", prerequisites: ["random-variables"], id: "r-random-variable-summary", topicId: "random-variables", unit: "statistics", order: 27, eyebrow: { zh: "第二十七课 · 随机变量", en: "Lesson 27 · Random variables" }, title: { zh: "计算离散随机变量的期望与方差", en: "Calculate expectation and variance" }, objective: { zh: "用概率质量函数计算期望和方差。", en: "Calculate expectation and variance from a PMF." }, explanation: { zh: "期望是长期平均，方差衡量围绕期望的波动。", en: "Expectation is a long-run average; variance measures spread." }, task: { zh: "保存 `expected_value` 和 `variance_value`。", en: "Save `expected_value` and `variance_value`." }, concepts: ["PMF", "expectation", "variance"], starterCode: "values <- 0:3\\nprobabilities <- c(0.1, 0.2, 0.4, 0.3)\\nexpected_value <-\\nvariance_value <-", hint: { zh: "使用概率加权和。", en: "Use probability-weighted sums." }, solution: "values <- 0:3\\nprobabilities <- c(0.1, 0.2, 0.4, 0.3)\\nexpected_value <- sum(values * probabilities)\\nvariance_value <- sum((values - expected_value)^2 * probabilities)", checkCode: "abs(expected_value - 1.9) < 1e-12 && abs(variance_value - 1.09) < 1e-12", success: { zh: "完成：期望和方差都由 PMF 加权得到。", en: "Complete: expectation and variance are weighted by the PMF." } },
  { language: "r", prerequisites: ["t-distribution"], id: "r-t-distribution-quantiles", topicId: "t-distribution", unit: "statistics", order: 28, eyebrow: { zh: "第二十八课 · t 分布", en: "Lesson 28 · t distribution" }, title: { zh: "比较不同自由度的临界值", en: "Compare critical values across degrees of freedom" }, objective: { zh: "理解自由度如何影响 t 分布尾部。", en: "Understand how degrees of freedom affect t tails." }, explanation: { zh: "自由度越小，尾部越厚。", en: "Smaller degrees of freedom produce heavier tails." }, task: { zh: "保存两个自由度下的临界值。", en: "Save critical values for two degrees of freedom." }, concepts: ["qt()", "degrees of freedom"], starterCode: "critical_df5 <-\\ncritical_df50 <-", hint: { zh: "使用 `qt(0.975, df)`。", en: "Use `qt(0.975, df)`." }, solution: "critical_df5 <- qt(0.975, df = 5)\\ncritical_df50 <- qt(0.975, df = 50)", checkCode: "critical_df5 > critical_df50", success: { zh: "完成：自由度越大，t 分布越接近正态分布。", en: "Complete: larger degrees of freedom make t closer to normal." } },
  { language: "r", prerequisites: ["sampling-distributions"], id: "r-sampling-distribution", topicId: "sampling-distributions", unit: "statistics", order: 29, eyebrow: { zh: "第二十九课 · 抽样分布", en: "Lesson 29 · Sampling distributions" }, title: { zh: "模拟样本均值的标准误", en: "Simulate the standard error of a mean" }, objective: { zh: "区分样本量、重复抽样次数和样本均值的变异。", en: "Distinguish sample size, repetitions, and variation in sample means." }, explanation: { zh: "理论标准误是总体标准差除以样本量平方根。", en: "The theoretical standard error is population spread divided by sqrt(n)." }, task: { zh: "保存 1000 个样本均值的标准差 `observed_se`。", en: "Save the standard deviation of 1000 sample means." }, concepts: ["sampling distribution", "standard error"], starterCode: "set.seed(42)\\npopulation <- rnorm(100000, 10, 4)\\nmeans <- replicate(1000, mean(sample(population, 25)))\\nobserved_se <-", hint: { zh: "使用 `sd(means)`。", en: "Use `sd(means)`." }, solution: "set.seed(42)\\npopulation <- rnorm(100000, 10, 4)\\nmeans <- replicate(1000, mean(sample(population, 25)))\\nobserved_se <- sd(means)", checkCode: "length(means) == 1000 && observed_se > 0.6 && observed_se < 1", success: { zh: "完成：观察标准误接近理论值 0.8。", en: "Complete: the observed standard error is close to 0.8." } },
