...
const TextbookResourceCatalog = lazy(() => import("./course/components/TextbookResourceCatalog"));
const EditorialDemoRouter = lazy(() => import("./visual-demo/editorial/EditorialDemoRouter"));
...
export function CurrentPage() {
  if (
    ...
    return (
      <Suspense fallback={<div className="route-loading">Loading editorial demo…</div>}>
        <EditorialDemoRouter />
      </Suspense>
    );
  }
  if (window.location.pathname === "/") {
    return (
      <Suspense fallback={<div className="route-loading">Loading StatMind…</div>}>
        <PortalHome />
      </Suspense>
    );
  }
  if (window.location.pathname === "/profile" || window.location.pathname === "/profile/") {
    return (
      <Suspense fallback={<div className="route-loading">Loading profile…</div>}>
        <ProfilePage />
      </Suspense>
    );
  }
  if (window.location.pathname === "/catalog" || window.location.pathname === "/catalog/") {
    return (
      <Suspense fallback={<div className="route-loading">Loading resource catalog…</div>}>
