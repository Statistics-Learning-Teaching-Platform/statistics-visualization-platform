...
const uiCopy = {
	...
	en: {
		...
		runtimeError:
			"Python could not run the code. Use the Console message to correct it.",
...
export function PythonLearningWorkspace() {
	...
	const [codes, setCodes] = useState<Record<string, string>>(() =>
		Object.fromEntries(
			pythonLessons.map((lesson) => [lesson.id, lesson.starterCode]),
		),
	);
	const [completed, setCompleted] = useState<string[]>(() => {
		const validIds = new Set(pythonLessons.map(({ id }) => id));
		return loadLearningProgress().completedPythonLessons.filter((id) =>
			validIds.has(id),
		);
	});
	...
	const activeLesson = useMemo(
		() =>
			pythonLessons.find((lesson) => lesson.id === activeId) ??
			pythonLessons[0],
		[activeId],
	);
	...
	async function execute(shouldCheck: boolean) {
		...
		const isCurrentView = () =>
			mountedRef.current && viewEpochRef.current === viewEpoch;
	...
	async function askTutor(suggestedQuestion?: string) {
		...
		try {
			const response = await authenticatedFetch(
				"/st-qselector/api/ai/python-tutor",
				{
					method: "POST",
