...
function compactTask(text: string) {
  return text
    ...
    .replace(
      /^Complete the last line so that the mean of `scores` is stored in `average_score`\.$/i,
      "Store the mean of `scores` in `average_score`.",
    )
    .replace(
      "把 `scores` 的均值保存为 `average_score`。",
      "把 `scores` 的平均值保存到 `average_score`。",
    );
}
  ...
  return (
    <EditorialDemoShell current={pageId} mode="workspace" siteMode="product">
      ...
      >
        ...
        <section className="ed-notebook">
          <header className="ed-notebook__header ed-lesson-hero">
            <div className="ed-lesson-meta">
              <span>
                {practiceLabel} {String(activeLesson.order).padStart(2, "0")}
              </span>
              <span>
                {language === "zh"
                  ? `第 ${lessonIndex + 1} / ${lessons.length} 题`
                  : `${lessonIndex + 1} / ${lessons.length}`}
              </span>
            </div>
            <h1 id="lesson-title">{activeLesson.title[language]}</h1>
            <p className="ed-lesson-task">
              <span className="ed-task-label">{language === "zh" ? "任务" : "Task"}</span>
              <span>{compactTask(activeLesson.task[language])}</span>
            </p>
          </header>
          ...
          <section className="ed-code-cell" data-engine-status={engineStatus}>
            <header>
              <div>
                ...
                <span aria-hidden="true" />
                <strong>
                  lesson-{String(activeLesson.order).padStart(2, "0")}.{kind === "r" ? "R" : "py"}
                </strong>
              </div>
              <small>
            ...
            <footer>
              ...
              <Button variant="ghost" disabled={isBusy} onClick={onResetCode}>
          ...
          <section
            className="ed-notebook-output ed-run-result"
            aria-live="polite"
          ...
          >
            <header>
              <div>
                <span>02</span>
                <p className="ed-kicker">Run Result</p>
              </div>
              <div className="ed-run-result__actions">
                ...
                {outputReady && review.kind !== "success" ? (
                  <Button variant="outline" disabled={isBusy} onClick={onCheck}>
                    {copy.check}
                  </Button>
                ) : null}
              </div>
            ...
            {runState === "idle" ? (
              <p>{language === "zh" ? "尚未运行代码" : "Run your code to see the result."}</p>
            ) : null}
            {runState === "error" ? (
              <pre className="ed-run-result__error">{consoleLines.join("\n")}</pre>
            ) : null}
            {runState === "success" ? (
              <div className="ed-live-output-stack">
                {plotNode}
                {consoleLines.length ? <pre>{consoleLines.join("\n")}</pre> : null}
              </div>
            ) : null}
            {environmentItems.length ? (
              <div className="ed-variable-disclosure">
                ...
                >
                  <span aria-hidden="true">{showVariables ? "▾" : "▸"}</span>
                  {language === "zh" ? "查看变量" : "View variables"} {environmentItems.length}
                </button>
                {showVariables ? (
                  <dl className="ed-variable-list">
                    {environmentItems.map((item) => (
                      <div key={`${item.name}-${item.detail ?? ""}`}>
                        <dt>{item.name}</dt>
                        <dd>{derivedEnvironmentDetail(item, code, consoleLines, copy.runtime)}</dd>
                      </div>
                    ))}
                  </dl>
                ) : null}
            ...
            {review.kind === "failure" ? (
              <p className="ed-run-result__message ed-run-result__message--error">
                {review.message}
              </p>
            ) : null}
          </section>
          ...
          <section className="ed-help-disclosure">
            <button type="button" onClick={revealHint} disabled={hintStage >= 3}>
              ...
              <span>{hintStage}/3</span>
            </button>
            {hintStage === 1 ? (
              <p>
                {language === "zh"
                  ? "先确认要保存的是一个计算结果：左侧对象名已经给出，想想右侧应该得到什么。"
                  : "First identify the value to save: the object name is already on the left, so focus on the result the right side should produce."}
              </p>
            ) : null}
            {hintStage === 2 ? (
              <p>
                {language === "zh"
                  ? `可以使用 ${hintFunction ?? "对应的统计函数"}，把它应用到题目中的数据。`
                  : `Try ${hintFunction ?? "the relevant function"} on the data from the task.`}
              </p>
            ) : null}
          ...
          {review.kind === "success" ? (
            <section
              className="ed-quick-review"
              aria-label={language === "zh" ? "快速检查" : "Quick review"}
            >
              <header>
                <div>
                  <CheckIcon aria-hidden="true" />
                  <span>
                    <strong>{language === "zh" ? "完成本题" : "Lesson complete"}</strong>
                    <small>Success feedback</small>
                  </span>
                </div>
              </header>
              <p className="ed-quick-review__result">{review.message}</p>
              {environmentItems.some((item) => item.name === "average_score") ? (
                <p className="ed-quick-review__value">
                  <code>average_score</code> ={" "}
                  {derivedEnvironmentDetail(
                    environmentItems.find(
                      (item) => item.name === "average_score",
                    ) as EditorialEnvironmentItem,
                    code,
                    consoleLines,
                    "—",
                  )}
              ...
              <div className="ed-quick-review__question">
                ...
                <div
                  className="ed-review-options"
                  role="radiogroup"
                  aria-label={language === "zh" ? "选择一个答案" : "Choose an answer"}
                >
                  {reviewOptions.map((option, index) => (
                    <button
                      ...
                      type="button"
                      role="radio"
                      data-selected={reviewChoice === index || undefined}
                      aria-checked={reviewChoice === index}
                      onClick={() => setReviewChoice(index)}
                    >
                      {option}
                    </button>
                  ))}
                ...
                {reviewChoice !== null ? (
                  <p className="ed-review-response" aria-live="polite">
                    {reviewChoice === 1
                      ? language === "zh"
                        ? "正确。赋值会把右侧结果保存到左侧对象。"
                        : "Correct. Assignment saves the right-hand result in the left object."
                      : language === "zh"
                        ? "再想想：`<-` 会把结果保存到一个对象中。"
                        : "Think again: `<-` saves a result in an object."}
                  </p>
                ) : null}
              </div>
              {reviewChoice === 1 && nextLesson ? (
                <Button className="ed-next-lesson" onClick={() => onSelectLesson(nextLesson.id)}>
                  {language === "zh" ? "下一课" : "Next lesson"}
      ...
      {tutorOpen ? (
        <aside
          id={aiDrawerId}
          className="ed-ai-drawer"
          role="dialog"
          aria-modal="false"
          aria-labelledby={aiTitleId}
        >
          <header className="ed-ai-drawer__header">
            <div>
              <span aria-hidden="true">AI</span>
              <div>
                <p className="ed-kicker">{englishTitle}</p>
                <h2 id={aiTitleId}>{copy.tutor}</h2>
              </div>
            </div>
            <button
              ref={tutorCloseRef}
          ...
          {tutorGate ? (
            ...
            <>
                  ...
                  <div className="ed-tutor-note">
                    ...
                    <div className="ed-live-suggestions">
                      ...
                      <button
                        type="button"
                        onClick={() =>
                          onAskTutor(language === "zh" ? "<- 是什么意思？" : "What does <- mean?")
                        }
                      >
                        {language === "zh" ? "<- 是什么意思？" : "What does <- mean?"}
                      </button>
                      <button type="button" onClick={() => onAskTutor(copy.explainError)}>
                        {copy.explainError}
                      </button>
                    </div>
                  </div>
              ...
              >
                <textarea
                  value={tutorPrompt}
                  onChange={(event) => onTutorPromptChange(event.target.value)}
                  onKeyDown={(event) => {
                  ...
                  aria-label={copy.tutorPlaceholder}
                />
                <button
                  type="submit"
