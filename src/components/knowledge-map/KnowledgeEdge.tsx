...
export function KnowledgeEdge({ edge, source, target, isRelated, isDimmed }: KnowledgeEdgeProps) {
	...
	const paths = compare ? [edgePath(source, target, -4), edgePath(source, target, 4)] : [edgePath(source, target)];
	const labelX = (source.x + target.x) / 2;
	const labelY = (source.y + target.y) / 2 - 8;

	return (
		<g className="knowledge-map-edge" data-related={isRelated || undefined} data-dimmed={isDimmed || undefined}>
			...
			{edge.label && <text x={labelX} y={labelY} className="knowledge-map-edge__label">{edge.label}</text>}
		</g>
	);
}
