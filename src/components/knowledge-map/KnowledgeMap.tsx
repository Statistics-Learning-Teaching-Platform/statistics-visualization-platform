...
import type {
  KnowledgeEdge as KnowledgeEdgeData,
  KnowledgeNode as KnowledgeNodeData,
} from "../../data/knowledgeMaps";
import { type ChapterKnowledgeMap, knowledgeMaps } from "../../data/knowledgeMaps";
...
import { type KnowledgeMapLayout, layoutKnowledgeMap } from "./knowledgeMapLayout";
...
function getPrimaryAncestor(nodeId: string, nodesById: Map<string, KnowledgeNodeData>) {
  ...
  while (current && current.level > 1 && !visited.has(current.id)) {
    ...
    const parent = [...nodesById.values()].find((candidate) =>
      candidate.children?.includes(current!.id),
    );
    current = parent;
  }
...
function getVisibleGraph(
  ...
  const visibleEdges = treeEdges.filter(
...
export function KnowledgeMap({ chapterNumber, chapter, entry, language }: KnowledgeMapProps) {
  ...
  const visibleGraph = useMemo(
    () =>
      map
        ? getVisibleGraph(map, expandedNodeIds, activeNodeId)
  ...
  const layout = useMemo(
    () =>
      map
        ? layoutKnowledgeMap(visibleGraph.visibleNodes, visibleGraph.visibleEdges)
        : getFallbackMap(chapter),
    [chapter, map, visibleGraph],
  );
  const [zoom, setZoom] = useState(() =>
    typeof window !== "undefined" && window.innerWidth < 768 ? 1.2 : 1,
  );
  ...
  const relatedNodes = useMemo(
    () => directConnections(focusId, layout.edges),
  ...
  return (
    <section
      className="chapter-hub-section chapter-hub-knowledge"
      aria-labelledby="chapter-hub-knowledge-title"
    >
      <div className="chapter-hub-section__heading">
        <p className="chapter-hub-eyebrow">01 · KNOWLEDGE MAP</p>
        <h2 id="chapter-hub-knowledge-title">
          {language === "zh" ? "本章知识地图" : "Chapter knowledge map"}
        </h2>
        <p>
          {map?.subtitle ??
            (language === "zh"
              ? "先看章节骨架，再选择一个知识模块逐步展开。"
              : "Start with the chapter skeleton, then open one knowledge branch at a time.")}
        </p>
      </div>
        ...
        <div className={`knowledge-map-canvas-shell${dragging ? " is-dragging" : ""}`}>
          <svg
            ...
            aria-label={
              language === "zh"
                ? "可拖动、可缩放的本章知识关系图"
                : "Draggable and zoomable chapter knowledge relationship map"
            }
            ...
            onWheel={(event) => {
              ...
              setZoom((current) =>
                Math.min(
                  1.65,
                  Math.max(0.55, Number((current + (event.deltaY < 0 ? 0.1 : -0.1)).toFixed(2))),
                ),
              );
            }}
          >
            <defs>
              ...
              <marker
                id="knowledge-map-arrow-reverse"
                markerWidth="8"
                markerHeight="8"
                refX="1"
                refY="4"
            ...
            <rect
              className="knowledge-map-canvas__background"
              x="0"
              y="0"
              width={layout.width}
              ...
              {layout.edges.map((edge) => {
                ...
                return (
                  <KnowledgeEdge
                    ...
                    target={target}
                    isRelated={related}
                    isDimmed={Boolean(focusId && !related)}
                  />
                );
              })}
              {layout.nodes.map((node) => {
                ...
                return (
                  <KnowledgeNode
                    ...
                    isRelated={related}
                    isDimmed={Boolean(focusId && !related)}
                    isExpandable={node.level > 0 && Boolean(node.children?.length)}
          ...
          <KnowledgeMapControls
            language={language}
            onZoomIn={() =>
              setZoom((current) => Math.min(1.65, Number((current + 0.1).toFixed(2))))
            }
            onZoomOut={() =>
              setZoom((current) => Math.max(0.55, Number((current - 0.1).toFixed(2))))
            }
            onFitView={() => {
              setZoom(1);
              setPan({ x: 0, y: 0 });
          ...
          />
          <p
            className={`knowledge-map-canvas__hint${showCanvasHint ? "" : " is-faded"}`}
            aria-hidden={!showCanvasHint}
          >
            {language === "zh"
              ? "拖动探索 · Ctrl/⌘ + 滚轮缩放 · 点击空白清除"
              : "Drag to explore · Ctrl/⌘ + scroll to zoom · click blank space to clear"}
          </p>
        </div>
