...
function wrapLabel(label: string, maxLength: number) {
	...
	return [label.slice(0, maxLength), `${label.slice(maxLength, maxLength * 2)}${label.length > maxLength * 2 ? "…" : ""}`].filter(Boolean);
}

export function KnowledgeNode({ node, isActive, isRelated, isDimmed, isExpandable, isExpanded, onSelect, onHover }: KnowledgeNodeProps) {
	...
	const labelStart = node.description && variant === "root" ? -4 : (labelLines.length > 1 ? -((labelLines.length - 1) * 9) : 4);
	...
	return (
		...
		>
			...
			/>
			{variant === "root" && (
				<text x={-node.width / 2 + 16} y={-node.height / 2 + 20} className="knowledge-map-node__eyebrow">
					CHAPTER
				</text>
			)}
			<text x={-node.width / 2 + 16} y={labelStart} className="knowledge-map-node__label">
				{labelLines.map((line, index) => (
					<tspan key={`${node.id}-${index}`} x={-node.width / 2 + 16} dy={index === 0 ? 0 : 18}>{line}</tspan>
				))}
			</text>
			{node.description && variant === "root" && (
				<text x={-node.width / 2 + 16} y={node.height / 2 - 17} className="knowledge-map-node__description">
					{wrapLabel(node.description, 28).slice(0, 1)}
				</text>
			)}
			{isExpandable && (
				<text x={node.width / 2 - 18} y={-node.height / 2 + 22} className="knowledge-map-node__toggle" aria-hidden="true">
					{isExpanded ? "−" : "+"}
				</text>
			)}
		</g>
