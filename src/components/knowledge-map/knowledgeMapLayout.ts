...
function subtreeHeight(
  ...
  return Math.max(
    ...
    children.reduce(
      (total, child) => total + subtreeHeight(child, nodesById, edgesBySource, nextStack),
      0,
    ) +
 ...
 */
export function layoutKnowledgeMap(
  nodes: KnowledgeNode[],
  edges: KnowledgeEdge[],
): KnowledgeMapLayout {
  ...
  const placeSubtree = (node: KnowledgeNode, depth: number, side: -1 | 1, centerY: number) => {
    ...
    const children = childrenFor(node.id, nodesById, edgesBySource).filter(
      (child) => !placed.has(child.id),
    );
    if (children.length === 0) return;
    const heights = children.map((child) => subtreeHeight(child, nodesById, edgesBySource));
    const totalHeight =
      heights.reduce((sum, height) => sum + height, 0) + Y_GAP * (children.length - 1);
  ...
  if (root) {
    ...
    placed.add(root.id);
    const rootChildren = childrenFor(root.id, nodesById, edgesBySource).filter(
      (child) => child.level === 1,
    );
    ...
    const placeSide = (sideNodes: KnowledgeNode[], side: -1 | 1) => {
      ...
      const total =
        ...
        Y_GAP * Math.max(0, sideNodes.length - 1);
      let cursor = -total / 2;
