...
function chapterIntroduction(
	...
	return {
		...
		keyPoints: [
			language === "zh" ? "点击一级模块，查看它的核心概念。" : "Open a primary module to see its core concepts.",
			language === "zh" ? "有加号的节点还可以继续展开。" : "A plus sign means that a node can be opened further.",
		],
	};
}
...
function selectedExplanation(
	...
	return node.explanation ?? {
		summary:
			...
			(language === "zh" ? "这个知识点连接本章中的相关概念。" : "This concept connects related ideas in the chapter."),
	};
}

export function ConceptExplanationSection({ node, entry, chapter, chapterNumber, language }: ConceptExplanationSectionProps) {
	const explanation = node ? selectedExplanation(node, entry, language) : chapterIntroduction(chapter, entry, language);
	const title = node?.label ?? chapter.title;
	const englishLabel = node?.englishLabel;
	return (
		<section className="knowledge-map-explanation" aria-labelledby="knowledge-map-explanation-title">
			<div className="knowledge-map-explanation__copy">
				<p className="chapter-hub-eyebrow">{node ? "CONCEPT EXPLANATION" : `CHAPTER ${chapterNumber} · INTRODUCTION`}</p>
				...
				{explanation.paragraphs?.map((paragraph) => <p className="knowledge-map-explanation__paragraph" key={paragraph}>{paragraph}</p>)}
				...
				{explanation.formula && (
					<div className="knowledge-map-explanation__formula" aria-label={language === "zh" ? "公式" : "Formula"}>{explanation.formula}</div>
				)}
			</div>
