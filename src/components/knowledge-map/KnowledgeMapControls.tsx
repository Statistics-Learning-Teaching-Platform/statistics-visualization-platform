...
export function KnowledgeMapControls({
  ...
  return (
    <div
      className="knowledge-map-controls"
      aria-label={language === "zh" ? "知识地图控件" : "Knowledge map controls"}
    >
      <div className="knowledge-map-controls__group">
        <button type="button" onClick={onZoomIn} aria-label={labels.zoomIn} title={labels.zoomIn}>
      ...
      <div className="knowledge-map-controls__group">
        <button type="button" onClick={onFitView} aria-label={labels.fit} title={labels.fit}>
          <Maximize2Icon aria-hidden="true" />
        </button>
        <button type="button" onClick={onReset} aria-label={labels.reset} title={labels.reset}>
