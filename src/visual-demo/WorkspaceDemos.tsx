...
const catalogResources = {
  "06": {
    ...
    visualizations: ["中心极限定理实验", "置信区间覆盖实验", "置信区间案例实验"],
    r: ["模拟重复抽样", "构造均值置信区间", "手动计算 t 区间", "比较区间宽度"],
    python: ["抽样分布模拟", "均值置信区间", "样本量规划", "比例置信区间", "比较置信水平"],
  },
  "07": {
    title: "总体均值的比较",
    section: "7.1 假设检验",
    ...
    python: ["单样本检验", "两独立样本检验", "配对检验", "单因素 ANOVA", "事后比较"],
  },
} as const;
    ...
    <DemoPage current="catalog" className="sm-catalog-demo">
      <div className="sm-catalog-frame">
        ...
        <article className="sm-catalog-article">
          ...
          <section className="sm-learning-goals" aria-labelledby="catalog-goals">
          ...
          <section className="sm-reading-section">
            ...
            <div className="sm-equation-line" role="img" aria-label="估计值加减临界值乘以标准误">
              <span>估计值</span>
              <b>±</b>
              <span>临界值</span>
              <b>×</b>
            ...
            <div className="sm-textbook-note">
              <span>读图提示</span>
              <p>
                95% 描述的是方法在长期重复抽样中的覆盖表现，不是“这个固定区间有 95% 概率包含均值”。
              </p>
            </div>
          </section>

          <section className="sm-mini-figure" aria-labelledby="sampling-figure-title">
            <div className="sm-mini-figure__copy">
              <span>FIGURE 6.2</span>
              <h3 id="sampling-figure-title">同一总体，五次不同的样本</h3>
              <p>每一条线都是一次独立抽样得到的区间。蓝色虚线表示未知但固定的总体均值。</p>
            </div>
            <svg viewBox="0 0 500 210" role="img" aria-label="五条置信区间，其中一条未覆盖总体均值">
              <path className="grid" d="M55 185H470M120 34v151M260 34v151M400 34v151" />
              ...
              {[55, 86, 117, 148, 179].map((y, index) => {
                ...
                return (
                  <g key={y} className={missed ? "missed" : "covered"}>
                    ...
                    <circle cx={(starts[index] + ends[index]) / 2} cy={y} r="4" />
                    {missed ? (
                      <text x="78" y={y - 9}>
                        未覆盖
                      </text>
...
export function ExperimentVisualDemo() {
  ...
  const visibleRows = intervalRows.slice(0, Math.min(generated, intervalRows.length));
  const covered = visibleRows.filter((row) => row.covered).length;
  const coverage = visibleRows.length ? ((covered / visibleRows.length) * 100).toFixed(1) : "—";
        ...
        <aside className="sm-experiment-nav">
          ...
          <nav>
            {[
              ...
              <button key={title} type="button" className={index === 3 ? "is-active" : undefined}>
          ...
          <div className="sm-experiment-steps">
            ...
            {[
              ...
              <div key={String(number)} className={done ? "is-done" : undefined}>
                <span>{number}</span>
                <b>{title}</b>
              </div>
            ))}
        ...
        <aside className="sm-experiment-controls">
          ...
          <div className="sm-control-group">
            ...
            <label>
              ...
              <input type="range" min="1" max="4" step="0.5" value="2" readOnly />
            </label>
          </div>
          <div className="sm-control-group">
            <p>抽样设定</p>
          ...
          <div className="sm-control-actions">
            <button type="button" onClick={() => setGenerated((value) => Math.min(value + 1, 12))}>
              ＋ 生成 1 个样本
            </button>
...
export function PaperVisualDemo() {
  ...
  const selectedQuestions = demoQuestions.filter((question) => selected.includes(question.id));
  const totalPoints = selectedQuestions.reduce((sum, question) => sum + question.points, 0);
  const totalMinutes = selectedQuestions.reduce((sum, question) => sum + question.minutes, 0);
      ...
      <div className="sm-paper-frame">
        <aside className="sm-paper-filters">
          ...
          <fieldset>
            <legend>教材章节</legend>
            <label>
              <input type="checkbox" defaultChecked /> 第 6 章 · 样本推断 <span>34</span>
            </label>
            <label>
          ...
          <fieldset>
            <legend>题型</legend>
            {["计算题 142", "综合题 98", "简答题 39", "选择题 7"].map((item, index) => (
              <label key={item}>
        ...
        <section className="sm-question-bank">
          ...
          <div className="sm-question-list">
            {demoQuestions.map((question, index) => {
              ...
              return (
                <article key={question.id} className={isSelected ? "is-selected" : undefined}>
        ...
        <aside className="sm-current-paper">
          <div className="sm-current-paper__head">
            ...
            <input aria-label="试卷名称" defaultValue="统计学基础 · 期中考试卷 A" />
          </div>
...
const rIde: IdeConfig = {
  ...
  groups: [
    {
      name: "01 · R 编程基础",
      lessons: ["保存一组成绩并计算均值", "认识 RStudio 与可复现脚本", "筛选满足条件的观测"],
    },
    {
      name: "02 · 数据与图形",
  ...
  output: "[1] 80.8",
  explanation: "mean() 对五个观测求和并除以观测数；average_score 保存了结果 80.8。",
  variables: [
...
const pythonIde: IdeConfig = {
  ...
  explanation: "布尔筛选保留 88、91、85；groupby() 按 A/B 分组后计算均值。B 组样本均值更高。",
  variables: [
...
function LearningIdeVisualDemo({ config }: { config: IdeConfig }) {
  ...
  const [tool, setTool] = useState<"Tutor" | "Console" | "Variables" | "Review">("Tutor");
  ...
  return (
    <DemoPage current={config.page} className={`sm-ide-demo sm-ide-demo--${config.page}`}>
      ...
      <div className="sm-ide-frame">
          ...
          {config.groups.map((group) => (
            <section key={group.name}>
              ...
              {group.lessons.map((lesson, index) => {
                ...
                return (
                  <button key={lesson} type="button" className={active ? "is-active" : undefined}>
                    <span>{active ? "●" : index < 1 ? "✓" : "○"}</span>
                    <span>
                      <b>{lesson}</b>
                      <small>{active ? "当前课程" : index < 1 ? "已完成" : "未开始"}</small>
                    </span>
                  </button>
                );
              })}
        ...
        <section className="sm-notebook">
          ...
          <section className="sm-code-cell">
            <header>
              <span>
                ...
                <i className="is-green" /> {config.page === "r" ? "R" : "Python"} 代码单元
              </span>
          ...
          <section className={`sm-output-cell ${hasRun ? "has-output" : ""}`}>
            ...
            {hasRun ? <pre>{config.output}</pre> : <p>运行代码后，结果会显示在这里。</p>}
          </section>
          ...
          <section className="sm-notebook-block sm-notebook-explanation">
            ...
            <div>
              <h3>结果解释</h3>
              <p>{hasRun ? config.explanation : "先运行代码，再把输出与统计概念联系起来。"}</p>
            </div>
          </section>
          <section className={`sm-check-cell ${hasRun ? "is-passed" : ""}`}>
            ...
            <div>
              ...
              <small>{hasRun ? "对象、数值与任务要求一致" : "完成代码后运行自动检查"}</small>
            </div>
            <button type="button" onClick={runExample}>
              {hasRun ? "进入下一课" : "检查答案"}
            </button>
        ...
        <aside className="sm-ide-tools">
          <div className="sm-tool-tabs" role="tablist" aria-label="学习工具">
            {(["Tutor", "Console", "Variables", "Review"] as const).map((item) => (
              <button
                key={item}
                type="button"
                role="tab"
          ...
          {tool === "Review" ? (
            <div className="sm-review-panel">
              ...
              <p>{hasRun ? "本课证据已保存到学习记录。" : "运行代码并检查后显示详细结果。"}</p>
            </div>
          ) : null}
        </aside>
      </div>
