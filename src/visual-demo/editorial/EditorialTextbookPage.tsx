...
import { textbookChapters as demoTextbookChapters, type EditorialSiteMode } from "./demo-data";
import { ChapterHeader, EditorialDemoShell, FigureFrame } from "./EditorialPrimitives";
import {
...
function getInitialChapter(siteMode: EditorialSiteMode): TextbookDemoChapterNumber {
  ...
  return [
    {
      ...
      detail: language === "zh" ? "从章节概念检查开始" : "Begin with a chapter concept check",
  ...
  return {
    ...
    subtitleLines:
      language === "zh"
        ...
        : ["Begin with an observable question,", "then connect concept, method, and explanation."],
    lead: thesis,
    meta: [
      language === "zh" ? "预计阅读 20 分钟" : "20 min reading",
      language === "zh" ? `${entry.topics.length} 个知识点` : `${entry.topics.length} topics`,
      language === "zh" ? `${resources.length} 项配套资源` : `${resources.length} resources`,
    ],
    objectives:
      language === "zh"
        ? [
    ...
    sections: [
      {
        ...
        callout: {
          ...
          title:
            language === "zh" ? "把计算对象放回情境中" : "Return every quantity to its context",
          body:
            language === "zh"
      ...
      },
      {
        ...
        callout: {
          ...
          title: language === "zh" ? "报告检查过什么" : "Report what was checked",
          body:
            language === "zh"
...
function TextbookReadingRail({
  ...
  const availableChapters =
    siteMode === "product"
      ? chapterOptions
      : chapterOptions.filter((item) => item.number === "06" || item.number === "07");
  const chapterGroups =
    siteMode === "product"
      ? [
          {
      ...
      : [
          {
            title: language === "zh" ? "可预览章节" : "Preview chapters",
            chapters: availableChapters,
          },
        ];
  return (
    <aside className="ed-textbook-sidebar ed-textbook-reading-rail" aria-label="教材章节目录">
      ...
      {siteMode === "product" && onBackToCatalog ? (
        <button className="ed-textbook-reading-rail__back" type="button" onClick={onBackToCatalog}>
...
function CatalogStatisticalFigure() {
  return (
    <figure className="ed-catalog-hero__figure" aria-labelledby="catalog-figure-title">
      <figcaption>
  ...
  return (
    ...
    >
      {items.map(({ label, href, tone, Icon }) => (
        <a
          className={`ed-catalog-resource-nav__item ed-catalog-resource-nav__item--${tone}`}
          href={href}
          key={href}
        >
  ...
  return (
    ...
    >
      {items.map(({ value, label, detail, tone, Icon }) => (
        <div
          className={`ed-catalog-resource-summary__item ed-catalog-resource-summary__item--${tone}`}
          key={label}
        >
          <Icon aria-hidden="true" />
          <div>
...
function TextbookCatalogOverview({
  ...
  const groups = [
    {
      ...
      detail:
        language === "zh"
          ? "建立统计语言，学会描述数据与理解随机性。"
          : "Build statistical language, describe data, and understand randomness.",
    ...
    },
    {
      ...
      detail:
        language === "zh"
          ? "从样本走向总体，进入比较、回归、检验与时间序列。"
          : "Move from samples to populations through comparison, regression, testing, and time series.",
  ...
  const summaryValues = useMemo<CatalogResourceSummaryValues>(() => {
    ...
    return {
      ...
      questions:
        ...
        reviewedQuestionIndex.questions.filter(
          (question: { isReviewed?: boolean }) => question.isReviewed,
        ).length,
    };
  ...
  return (
    <main id="main-content" className="sm-page-container ed-catalog-overview">
      ...
      <div
        className="ed-catalog-overview__groups"
        aria-label={language === "zh" ? "完整教材章节目录" : "Complete textbook chapter index"}
      >
        {groups.map((group) => (
            ...
            <div className="ed-catalog-group__chapters">
              {group.chapters.map((item) => {
                ...
                const resourceCount = entry
                  ? entry.visualizations.length +
                    entry.rLessons.length +
                    entry.pythonLessons.length +
                    1
                  : 1;
                return (
                  <button
                  ...
                  >
                    ...
                    <span className="ed-catalog-chapter__meta">
                      {language === "zh"
                        ? `${resourceCount} 项配套资源`
                        : `${resourceCount} companion resources`}
                    </span>
                    <ArrowRightIcon aria-hidden="true" />
                  </button>
                );
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        {section.callout ? (
          <aside className="ed-textbook-key-idea" aria-label={section.callout.label}>
...
function MiniEvidenceLab({ chapter }: { chapter: TextbookDemoChapter }) {
  ...
  const scale = (value: number) =>
    80 + ((value - chapter.lab.axisMin) / (chapter.lab.axisMax - chapter.lab.axisMin)) * 560;
  ...
  return (
    <div className="ed-inline-evidence-lab" aria-labelledby={labTitleId}>
      ...
      <p className="ed-inline-evidence-lab__explanation">{chapter.lab.explanation}</p>
      ...
      <div className="ed-inline-evidence-lab__plot">
        ...
        >
          ...
          <path d="M80 154V166M360 154V166M640 154V166" className="ed-inline-lab-axis" />
          ...
          <text x={center} y="78" textAnchor="middle" className="ed-inline-lab-label">
    ...
    <div className="ed-textbook-practice-resources">
      ...
      <div>
        {chapter.resources.map((resource) => (
          <a key={`${resource.type}-${resource.href}-${resource.title}`} href={resource.href}>
  ...
  return (
    <aside className="ed-chapter-tools" aria-label="本章学习工具">
      ...
      <section className="ed-textbook-margin-note">
        <h3>阅读边注</h3>
        <label htmlFor={`chapter-${chapter.number}-note`}>写下尚未解释清楚的问题</label>
        <textarea
...
export function EditorialTextbookPage({ siteMode = "demo" }: { siteMode?: EditorialSiteMode }) {
  const language = useLanguage();
  ...
  const chapterOptions = useMemo<ChapterOption[]>(
    () =>
      siteMode === "product"
        ...
        : demoTextbookChapters.filter((option) => option.number === "06" || option.number === "07"),
    [language, siteMode],
  );
  ...
  const questionBankHref =
    siteMode === "product" && catalogEntry ? catalogEntry.questionBankHref : "/visual-demo/paper";
  ...
  const chapterIndex = chapterOptions.findIndex((option) => option.number === chapterNumber);
  ...
  return (
    <EditorialDemoShell
      current="catalog"
      mode={showCatalogOverview ? "document" : "workspace"}
      siteMode={siteMode}
    >
      {showCatalogOverview ? (
        ...
        <main id="main-content" className="ed-textbook-layout">
          ...
          <article className="ed-reading-sheet" id="chapter-start">
            <ChapterHeader number={chapter.number} title={chapter.title} subtitle={chapter.lead} />

            <section className="ed-learning-objectives" aria-labelledby="learning-goals-title">
            ...
            {chapter.sections.map((section) => (
              <ReadingSection key={section.slug} chapterNumber={chapter.number} section={section} />
            ))}
            ...
            >
              ...
              <div>
                ...
                <h2 id={`chapter-${chapter.number}-practice-title`}>让公式变成可以观察的证据</h2>
                <p>
            ...
            >
              ...
              <div>
                ...
                <h2 id={`chapter-${chapter.number}-exercise-title`}>检查你的理解</h2>
                <p>
                  先给出自己的解释，再展开提示与答案。每道题都对应本章的一条核心判断，而不是只检查公式记忆。
                </p>
                <ExerciseSet chapter={chapter} />
                <a className="ed-textbook-question-bank-link" href={questionBankHref}>
                  <BookOpenCheckIcon aria-hidden="true" />
                  <span>
            ...
            <nav className="ed-reading-pagination" aria-label="章节翻页">
              <a href="/visual-demo">← 返回完整教材目录</a>
              <div className="ed-reading-pagination__chapters">
                <Button
                  type="button"
                  variant="outline"
                  disabled={!previousChapter}
                  onClick={() => previousChapter && changeChapter(previousChapter.number)}
                >
                  {previousChapter ? `上一章：${previousChapter.title}` : "已是第一章"}
                </Button>
                <Button
                  type="button"
                  variant="outline"
