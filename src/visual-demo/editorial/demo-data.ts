export type EditorialPageId = "home" | "catalog" | "experiment" | "paper" | "r" | "python";
...
export function getEditorialNavItems(mode: EditorialSiteMode): EditorialNavItem[] {
...
export const rIdeConfig: IdeConfig = {
  ...
  output: "[1] 80.8",
  explanation: "mean() 汇总向量中的五个观测值；对象 average_score 让结果可以在后续分析中继续使用。",
...
export const pythonIdeConfig: IdeConfig = {
  ...
  explanation: "先用布尔条件保留及格记录，再按班级分组。均值需要和每组样本量、分布形态一起解释。",
  variables: ["df · DataFrame 30 × 4", "passed_df · DataFrame 24 × 4", "summary · Series 3"],
  realHref:
