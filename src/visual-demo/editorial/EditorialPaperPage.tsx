import { ArrowRightIcon, FileDownIcon, MinusIcon, PlusIcon, SearchIcon } from "lucide-react";
...
export function EditorialPaperPage() {
  ...
  const [selected, setSelected] = useState<string[]>([demoQuestions[0].id, demoQuestions[1].id]);
  const [query, setQuery] = useState("");
  const [selectedChapters, setSelectedChapters] = useState(["06", "07"]);
  const [selectedTypes, setSelectedTypes] = useState(["选择题", "计算题", "分析与简答"]);
  const visibleQuestions = useMemo(() => {
    ...
    return demoQuestions.filter((question) => {
      ...
      const typeGroup =
        question.type === "选择题" || question.type === "计算题" ? question.type : "分析与简答";
  ...
  function toggleFilter(value: string, setValues: Dispatch<SetStateAction<string[]>>) {
    setValues((current) =>
        ...
        <section className="ed-question-manuscript">
          ...
          <div className="ed-question-list">
            {visibleQuestions.map((question, index) => {
              ...
              return (
                <article key={question.id} data-selected={isSelected || undefined}>
