...
import { EditorialDemoShell, ExperimentPanel, FigureFrame, PageIntro } from "./EditorialPrimitives";

function ConfidenceIntervalPlot({ rows, confidence }: { rows: IntervalRow[]; confidence: number }) {
  const x = (value: number) => 82 + ((value - 6.5) / 7.5) * 638;

  return (
    <svg viewBox="0 0 800 520" role="img" aria-labelledby="ci-plot-title ci-plot-description">
        ...
        {rows.map((row, index) => {
          ...
          return (
            <g key={row.id} className={row.covered ? undefined : "is-missed"}>
              ...
              <path d={`M${x(row.low)} ${y - 6}V${y + 6}M${x(row.high)} ${y - 6}V${y + 6}`} />
...
export function EditorialExperimentPage() {
  ...
  const coverage = visibleCount ? ((covered / visibleCount) * 100).toFixed(1) : "0.0";
    ...
    <EditorialDemoShell current="experiment" mode="workspace">
      <main id="main-content" className="ed-lab-page">
        <aside className="ed-lab-sidebar">
          ...
          <nav aria-label="实验目录">
            {experimentItems.map(([number, title]) => (
              <a
                key={number}
                href={number === "05" ? "#experiment-stage" : "/teaching-platform"}
        ...
        <section className="ed-lab-stage" id="experiment-stage">
          ...
          <ExperimentPanel number="A" label="Observation" title="让区间累积，观察覆盖与遗漏">
            ...
            >
              {visibleCount ? (
                <ConfidenceIntervalPlot rows={visibleRows} confidence={confidence} />
              ) : (
        ...
        <aside className="ed-parameter-rail">
          ...
          <div className="ed-parameter-actions">
            <Button onClick={() => setVisibleCount((count) => Math.min(count + 1, 12))}>
