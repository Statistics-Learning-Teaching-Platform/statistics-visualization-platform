...
import { EditorialDemoShell, EditorialFooter } from "./EditorialPrimitives";
...
const englishWorkspaceCopy = [
  ...
  },
  {
    ...
    description: "Use pandas, NumPy, and visualization tools for reproducible data analysis.",
...
const workspacePresentation = [
  {
    ...
    Icon: FlaskConicalIcon,
    tone: "teal",
    count: { zh: "13 个实验模块", en: "13 modules" },
  },
  {
    key: "exam",
    Icon: FileCheck2Icon,
    tone: "teal",
    count: { zh: "296 道已审核题目", en: "296 reviewed questions" },
  },
  {
    key: "r",
    Icon: ChartNoAxesCombinedIcon,
    tone: "orange",
    count: { zh: "39 个编程实验", en: "39 coding labs" },
  },
  {
    key: "python",
    Icon: BracesIcon,
    tone: "purple",
    count: { zh: "43 个编程实验", en: "43 coding labs" },
  },
] as const;

function getResumeInfo(progress: ReturnType<typeof loadLearningProgress>, language: "zh" | "en") {
  ...
  const hasHistory = hasRecordedProgress || (storedRoute && storedRoute !== "/teaching-platform");
...
function HomeLearningJourney({ language }: { language: "zh" | "en" }) {
  return (
    <div
      className="ed-home-journey"
      aria-label={language === "zh" ? "五步学习证据链" : "Five-step learning evidence chain"}
    >
...
function HomeFeaturePoints({ language }: { language: "zh" | "en" }) {
  return (
    <div
      className="ed-home-features"
      aria-label={language === "zh" ? "平台核心能力" : "Platform capabilities"}
    >
...
function HomeTextbookBanner({ language, href }: { language: "zh" | "en"; href: string }) {
  const copy =
    language === "zh"
      ...
      : {
          ...
          description:
            "Read by knowledge structure with visual experiments, R / Python practice, and exercises.",
          facts: [
  ...
  return (
    <section className="ed-home-textbook" aria-labelledby="home-textbook-title">
      <div
        className="ed-home-textbook__cover"
        aria-label={language === "zh" ? "教材封面" : "Textbook cover"}
      >
      ...
      <a
        href={href}
        aria-label={language === "zh" ? "打开教材" : copy.action}
  ...
  return (
    <div className="ed-home-workspaces" aria-label="四个核心产品入口">
      {entries.map((entry, index) => {
        ...
        const title =
          presentation.key === "laboratory"
            ? t.laboratory
            : presentation.key === "exam"
              ? t.exam
              : presentation.key === "r"
                ? t.rStudio
                : t.pythonStudio;
        const legacyTitle =
          presentation.key === "laboratory"
            ...
            : presentation.key === "exam"
              ...
              : presentation.key === "r"
                ? "R 语言知识库"
                : presentation.key === "python"
                  ? "Python 语言知识库"
                  : title;
        const action = entry.action;
        return (
          <article
            className={`ed-home-workspace-row ed-home-workspace-row--${presentation.tone}`}
            data-secondary={presentation.key === "exam" || undefined}
            key={entry.href}
          >
            <span className="ed-home-workspace-row__icon">
              <Icon aria-hidden="true" />
            </span>
...
export function EditorialHomePage({ siteMode = "demo" }: { siteMode?: EditorialSiteMode }) {
  const language = useLanguage();
  const t = homeCopy[language];
  const progress = useMemo(() => loadLearningProgress(), []);
  const resume = useMemo(() => getResumeInfo(progress, language), [progress, language]);
  const catalogHref = siteMode === "product" ? "/catalog" : "/visual-demo/catalog";
