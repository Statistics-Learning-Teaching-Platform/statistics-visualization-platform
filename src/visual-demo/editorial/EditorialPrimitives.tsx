      ...
      <header className="ed-header">
        ...
        >
          ...
          <span className="ed-wordmark__text">
            ...
            <small>
              {language === "zh" ? "统计思维教学平台" : "Statistical thinking platform"}
            </small>
          </span>
        </a>
...
export function PageIntro({ index, eyebrow, title, description, action }: PageIntroProps) {
  return (
...
export function EditorialSection({
  ...
  return (
    <section className={cn("ed-section", className)}>
      <div className={cn("ed-section__heading", !index && "ed-section__heading--plain")}>
...
export function LearningJourney() {
  ...
  return (
    <div
      ...
      aria-label={language === "zh" ? "四步学习证据链" : "Four-step learning evidence chain"}
    >
      ...
      {journeySteps.map((step, index) => (
        <article key={step.title} className="ed-journey__step">
          ...
          <p>{language === "zh" ? step.description : englishSteps[index].description}</p>
        </article>
      ))}
    </div>
  );
...
export function TextbookSidebar({ chapters, current, onSelect, available }: TextbookSidebarProps) {
  return (
...
export function FigureFrame({ number, title, description, children }: FigureFrameProps) {
  return (
...
export function ExperimentPanel({ number, label, title, children }: ExperimentPanelProps) {
  return (
...
export function EditorialFooter({ siteMode = "demo" }: { siteMode?: EditorialSiteMode }) {
  const language = useLanguage();
  return (
